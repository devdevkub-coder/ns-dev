...
- บิลรับซื้อ Stock ต้องให้ office เลือก `คลัง` จาก dropdown ที่ filter ตามสาขาของบิลเท่านั้น; backend ต้อง validate ว่าคลัง active และอยู่สาขาเดียวกับบิลก่อนบันทึก `purchase_bills.warehouse_id` และ `stock_ledger.warehouse_id`
- ระบบห้าม auto-pick คลังให้บิลรับซื้อจากชื่อ, code, type, หรือ hint เช่น `RM/WIP/FG/SCRAP`; ถ้าไม่ได้เลือกคลังที่ถูกต้องต้อง reject การบันทึก ไม่ใช้ fallback
