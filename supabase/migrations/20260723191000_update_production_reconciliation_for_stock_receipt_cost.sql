...
po2_fact as (
  select
    ...
    sum(case when po2.category_code in ('FG', 'RM') then coalesce(po2.stock_receipt_total_cost, po2.total_cost, 0) else 0 end) as expected_stock_value
...
active_wip as (
  select
    ...
    coalesce(sum(pi.qty) filter (where pi.status = 'active'), 0)
      - coalesce((
        select sum(coalesce(pir.qty, 0))
        from public.production_input_returns pir
        where pir.order_id = po.id
...
select
  ...
  coalesce(po2_ledger.wip_value_out, 0) as actual_value,
  jsonb_build_object(
    ...
    'productionValue', po2_fact.expected_value,
    'expectedStockValue', po2_fact.expected_stock_value,
    'actualStockValue', coalesce(po2_ledger.output_value, 0)
  ) as details
from po2_fact
...
where abs(po2_fact.expected_wip_qty - coalesce(po2_ledger.wip_qty_out, 0)) > 0.000001
   ...
   or abs(po2_fact.expected_stock_value - coalesce(po2_ledger.output_value, 0)) > 0.000001
