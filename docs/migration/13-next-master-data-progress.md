...
- Tax rate settings are now DB-backed master/config lists. Dev-target has `vat_settings` seeded with `VAT 7%` and `wht_settings` seeded with `WHT 3%`; `/master-data/vat-settings` and `/master-data/wht-settings` expose name, percent rate, and active status. The shared master-data schema validates rate percent as `0-100`.
