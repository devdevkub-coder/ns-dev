...
| 2026-05-20 | Bank name/channel code removal follow-up | In progress | `/master-data/bank-names` and `/master-data/channels` no longer show or accept code fields. Prisma schema and API code are aligned to remove `code` from `bank_names`, `purchase_channels`, and `sales_channels`; dev-target DB drop/verification and final validation are pending in this batch |
