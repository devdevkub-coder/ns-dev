...
- `/dual-costing/cost-allocation-ledger` rendered its error state because the SIT database is behind the Prisma contract: `trading_allocation_facts` is missing `cost_pool_entry_id` and `target_ref_id`, producing `P2022`. The required schema is represented by `supabase/migrations/20260727110000_add_trading_allocation_fact_cost_pool_entry.sql`; applying it requires a separate environment/migration-history check and mutation authority. No UI fallback was added because the table contract is not the cause.
