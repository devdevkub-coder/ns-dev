...
- Correction note on 2026-05-30: the `voided PMA` direction above has been superseded by a newer business decision. Approved `PMA` rows must be treated as final and must not roll back to `ยกเลิกแล้ว`. `ยกเลิก` belongs only to the pre-approve draft decision in `/daily/payment-approval`, not to `/purchase/payments`.
- Correction note on 2026-05-30: `/purchase/payments` should now be treated as `PMT pending` rather than a plain PMA list. Approved PMA items feed the queue, but statuses such as `success / cancelled` belong to PMT/payment execution, not to PMA.
- Done on 2026-05-30: correction batch part 1 removed the PMA-cancel path from active code. `/daily/payment-approval` no longer reads `voided` approval rows, `/purchase/payments` no longer exposes queued-item cancel in the UI, `/api/purchase/payments/cancel-approved` now rejects requests instead of voiding PMA, and payment-history no longer fabricates `ยกเลิกแล้ว` rows from `payment_approvals.status='voided'`.
- Done on 2026-05-30: `Purchase Flow.md` state/use-case matrix now reflects the superseding rule explicitly: approved `PMA` is final, `/purchase/payments` is `PMT pending`, and `ยกเลิกแล้ว` belongs to `PMT`/voucher history only.
- Done smoke on 2026-05-30: local browser smoke passed for the corrected PMA-final path. `/daily/payment-approval` renders only `ทั้งหมด / ยังไม่อนุมัติ / อนุมัติแล้ว` with no `ยกเลิกแล้ว` PMA filter, and `/purchase/payments` renders the queue heading `รายการรอออก PMT` with `ทำจ่าย` only and no queued-item cancel action.
