...
- Current unmapped legacy slice: remote-legacy-only Dual Costing views are being restored after the sidebar fix. `/dual-costing/waiting-allocations`, `/dual-costing/cost-allocation-ledger`, and `/dual-costing/report` now have dedicated read-only Next pages/APIs and sidebar entries. They derive from existing `sales_bills`, `trading_deals`, and product/party lookups only; no allocation write, reverse, stock, GL, or statutory P&L behavior is added. Remaining unmapped view is `trackAssetOverview`.
