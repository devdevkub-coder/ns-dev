...
Objective: harden WTI/WTO private original-only upload and background thumbnail processing. Active batch now includes server metadata/pixel validation, atomic queued/stale-lease draining, ticket/bucket scope isolation, orphan cleanup cron, immutable private storage keys, concurrent batch progress aggregation, and no original fallback. Migration `20260810160000_add_weight_ticket_image_processing_jobs.sql` is required but has not been applied. Focused image suite is `33/33`, Prisma validation/generation, targeted/full lint, build and `git diff --check` pass. Full type-check still reports only unrelated `dialog.tsx` casing and `document-print-contract.test.ts` fixture errors. Do not push, promote, apply migration, or touch Production; next step is user review/approval of this batch and SIT rollout plan.
