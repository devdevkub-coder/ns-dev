...
- Current master-data follow-up: `/master-data/salespersons` now uses sale codes in the lowercase `sa001`-`sa999` format. The API normalizes manual input like `sales1`/`s1`/`sa1` to `sa001`, generates the next `saNNN` code, and rejects values outside the `sa001`-`sa999` range. `/master-data/suppliers` uses supplier codes in lowercase `su0001`-`su99999`; legacy-style `SUP1`/`s1`/`su1` inputs normalize to `su0001`.
