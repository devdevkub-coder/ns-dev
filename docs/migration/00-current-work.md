...
- Current reference master cleanup: `/master-data/bank-names` and `/master-data/channels` no longer expose business code fields in the table/form/API response surface. Dev-target `public.bank_names`, `public.purchase_channels`, and `public.sales_channels` should not keep separate `code` columns; their hidden primary keys remain for FK stability.
