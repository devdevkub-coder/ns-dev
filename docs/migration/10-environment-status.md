...
- Postflight after schema apply: `510/510` existing image assets have deterministic `download_storage_key` values and no original object was changed. Download backfill then completed on SIT with `ready=510/510`, `generated=457`, `skippedClaim=53`, `errors=0`, and no non-ready rows; derivative metadata is complete and the aggregate derivative byte size is `98,782,959`. Deployment and browser UAT remain pending. Production was not modified.
