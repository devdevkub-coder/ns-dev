...
When creating or editing a user, selecting a Role immediately derives the branch-access mode from the selected Role scopes: any selected `branch_scope=all` Role uses `ทุกสาขา`; scoped Roles such as `own`/`custom` use `เฉพาะสาขาที่เลือก`. The form clears old branch selections when the Role set changes, then shows the valid mode and branch controls immediately. The API keeps the same server-side rule and rejects a mismatched branch payload. This keeps the UI from leaving a valid new-user form in an unexplained `unset` state while preserving fail-closed branch authorization.
