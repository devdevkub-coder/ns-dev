...
The employee list opens on `active` by default. After creating an employee, the page clears the status restriction to `all` instead of forcing `pending`, so the administrator keeps the current search context and sees the full list without an automatic post-save filter. Creating an employee no longer sends an email automatically while the production email/redirect contract is unavailable; the Admin uses the temporary-password action after activation. When an administrator activates a pending employee, a temporary-password dialog opens immediately. The server generates a strong password, stores it only in Supabase Auth, displays it once, and marks `must_change_password`. The raw password is never stored in application tables or audit metadata. The action menu has one `จัดการรหัสผ่าน` entry for active users and the dialog provides the same temporary-password flow; it does not require a disable/enable cycle. Disabled users cannot receive credential actions. Password-link UI is intentionally unavailable while the production email/redirect contract is not ready.
