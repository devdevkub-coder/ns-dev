...
วันที่ของ event ใน timeline ทั้ง PO Buy และ PO Sell ต้องมาจาก `expectedDelivery` เท่านั้น. `po_sells.date` คือวันที่ล็อคราคาและ `created_at` คือวันที่สร้างเอกสาร จึงห้ามใช้แทนวันส่งมอบเมื่อค่า `expectedDelivery` หายหรือไม่ถูกต้อง; ข้อมูลต้นทางต้องถูกแก้ก่อนเชื่อถือการจัดลำดับ/aging ได้.
