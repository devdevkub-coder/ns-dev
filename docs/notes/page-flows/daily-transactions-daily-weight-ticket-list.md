...
- Realtime collaboration follow-up 2026-08-06: หลัง create/edit/confirm/cancel สำเร็จ server จะ broadcast invalidation event ที่มีเพียง `documentNo`, `changeType` และ `updatedAt`; list/detail จะโหลดข้อมูลล่าสุดผ่าน API ที่ตรวจสิทธิ์อีกครั้ง. ฟอร์มที่กำลังกรอกจะไม่ถูกทับ แต่จะแจ้งว่ามีผู้ใช้อื่นบันทึกแล้วให้ตรวจสอบก่อน save. Realtime เป็นตัวเร่งการเห็นข้อมูล ไม่ใช่ source of truth และถ้า Realtime ใช้งานไม่ได้การ save/โหลดปกติยังทำงานต่อ.
