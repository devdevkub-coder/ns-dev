...
- Line-level collaboration follow-up 2026-08-07: event ของการแก้ไขจะระบุ persisted `lineIds` ซึ่งแต่ละ line หมายถึงสินค้า+เต๋า. ฟอร์มส่ง baseline line IDs/versions และ `collaborationDeletedLineIds`; server จะ merge เฉพาะ line ที่ผู้ใช้ไม่ได้แก้ ตรวจ version ของ line ที่แก้หรือลบ และตรวจ header fields ที่แก้จาก baseline. ถ้าเป็นคนละสินค้า/เต๋าจะบันทึกพร้อมกันได้; ถ้าเป็น line เดียวกัน, stale delete หรือ header เดียวกันที่ถูกแก้แล้ว จะตอบ `409 Conflict` โดยไม่เขียนทับ. UI แสดง `มีข้อมูลใหม่` และไม่ reset local draft.

What is what: `collaborationBase*` คือหลักฐานว่า request เริ่มจากเอกสาร/รายการชุดใด, `lineId` คือหน่วยสินค้า+เต๋า, และ `version` คือ revision ของ line ที่ server ยอมรับล่าสุด. Why it has to be like this: คนสองคนสามารถกรอกสินค้าเดียวกันคนละเต๋าได้โดยไม่ทับกัน; การแก้เต๋าเดียวกันต้องหยุดเป็น conflict ที่มองเห็นได้ แทนการใช้ last-write-wins ที่ทำข้อมูลสูญหาย
