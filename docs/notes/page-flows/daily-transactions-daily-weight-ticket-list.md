...
- UI follow-up 2026-08-07: ปุ่ม `+ เพิ่มสินค้า` ใช้สีเขียวและอยู่เฉพาะหัวข้อรายการสินค้า เพื่อให้ผู้ใช้เพิ่มรายการจาก list แล้วจึงเข้าไปกรอกข้อมูลใน editor บนมือถือ/แท็บเล็ต; ไม่แสดงปุ่มเพิ่มสินค้าซ้ำในหัวแผง editor. ปุ่ม `+ เพิ่มเต๋า` อยู่ใต้เต๋าล่าสุดของสินค้านั้นเพื่อให้เห็นจุดเพิ่มต่อเนื่องและกดเพิ่มจาก parent เดิมได้, ปุ่ม `+ เพิ่มสิ่งเจือปน` อยู่ในหัวข้อสิ่งเจือปนหรือใน empty state เมื่อยังไม่มีรายการหัก และปุ่ม `บันทึกสินค้านี้` อยู่ใน action bar มือถือแทนปุ่มเพิ่มเต๋า; desktop ยังคงแสดงปุ่มบันทึกใน section. แต่ละเต๋าแสดงเป็น section กรอบแยกพร้อมหัวข้อ `รายละเอียดเต๋าที่ ...` และยังยุบ/ขยายได้
- เมื่อเพิ่มรายการสินค้าใหม่แต่ยังไม่เลือกสินค้า ให้แสดงเฉพาะส่วนเลือกสินค้า; section เต๋า สิ่งเจือปน สรุปน้ำหนัก และหมายเหตุจะแสดงหลังเลือกสินค้าแล้วเท่านั้น เพื่อไม่ให้ผู้ใช้กรอกข้อมูลในรายการที่ยังไม่มีสินค้า
- เมื่อกดบันทึกแล้วพบ validation error ในเต๋าหรือสิ่งเจือปน ระบบต้องเปิดเต๋า/สิ่งเจือปนต้นทางที่ผิดให้เห็น เลื่อนหน้าจอไปยังช่องแรกที่ผิด และ focus ช่องนั้น แม้ผู้ใช้จะยุบเต๋าไว้ก่อนกดบันทึก เพื่อให้แก้ข้อมูลต่อได้ทันที
- ผู้ดำเนินการที่แสดงใน WTI ใช้ชื่อพนักงานจาก `app_users.display_name` สำหรับ `ผู้สร้างเอกสาร`, `อัปเดตล่าสุด`, timeline, ประวัติการใช้งาน และ downstream allocation; ค่า email ที่ persist ใน audit field ยังคงเป็น source ภายในเพื่อ traceability. ถ้า actor หรือ creator ไม่มีข้อมูลที่ contract บังคับ ต้องแจ้งข้อมูลไม่สมบูรณ์และหยุด response ห้ามใช้ field อื่นแทนหรือ hardcode ค่าแสดงผล
- กรณีข้อมูล provenance/timeline ไม่ครบ API ต้องตอบ data-contract error ที่ระบุสาเหตุและหยุดการแสดงผลข้อมูล ห้ามกลบเป็นค่าผู้สร้าง/ผู้แก้ไขคนอื่น และห้ามแปลงเป็นข้อความ placeholder ใน source response
- Timeline WTI/WTO เป็น append-only snapshot: event เดิมไม่ถูกแก้, `created` คงผู้สร้างเดิม, การแก้ไข/ออกบิล/คืนรายการเพิ่ม event ใหม่ และการบันทึกซ้ำที่ไม่มี business change จะไม่เพิ่ม `edited` event
...
- สิ่งเจือปนทุกชนิดบนมือถือ รวมถึง `สินค้าอื่น` และรายการซ้อนระดับถัดไป เป็น card ที่ยุบ/ขยายได้เหมือนเต๋า: ตอนยุบแสดงลำดับ ชื่อสิ่งเจือปน ยอดหัก และสถานะครบ/ไม่ครบ; เพิ่มสิ่งเจือปนใหม่จะยุบ card เดิมของสินค้านั้นและเปิด card ใหม่ให้กรอกต่อ. ปุ่มลบมีเฉพาะขณะ card ขยาย เพื่อไม่ให้ลบผิดจากหน้าสรุป; เมื่อขยายจะแสดงฟิลด์สินค้า ซื้อ/ไม่ซื้อ คำเตือน และรูปประกอบตามชนิดรายการเดิม
- เมื่อกรอกสิ่งเจือปนครบแล้ว ให้แสดงปุ่ม `หักสิ่งเจือปนต่อ` เป็นแถวเต็มใต้รายการนั้นอย่างชัดเจน (ไม่เบียดรวมกับคอลัมน์ลบ); ปุ่มนี้สร้างรายการลูกด้วย `parentId` ของสิ่งเจือปนเดิมและเปิด editor ให้กรอกการหักซ้ำต่อได้ทั้ง WTI และ WTO
...
- Target design — line-level collaboration (ยังไม่ใช่ runtime ของ Production baseline): event ของการแก้ไขควรระบุ persisted `lineIds` ซึ่งแต่ละ line หมายถึงสินค้า+เต๋า. ฟอร์มควรส่ง baseline line IDs/versions และรายการ line ที่ลบ; server ต้อง merge เฉพาะ line ที่ผู้ใช้ไม่ได้แก้ ตรวจ version ของ line ที่แก้หรือลบ และตรวจ header fields ที่แก้จาก baseline. เป้าหมายคือคนละสินค้า/เต๋าบันทึกพร้อมกันได้ ส่วน line หรือ header เดียวกันที่เปลี่ยนจาก baseline ต้องตอบ `409 Conflict` โดยไม่เขียนทับ. ต้องเพิ่มกลับแบบ incremental พร้อม test สอง session ก่อนเปลี่ยนข้อความนี้เป็น active runtime.

Target terminology: `collaborationBase*` คือหลักฐานว่า request เริ่มจากเอกสาร/รายการชุดใด, `lineId` คือหน่วยสินค้า+เต๋า, และ `version` คือ revision ของ line ที่ server ยอมรับล่าสุด. เหตุผลที่ต้องเพิ่มกลับภายหลัง: คนสองคนควรกรอกสินค้าเดียวกันคนละเต๋าได้โดยไม่ทับกัน และการแก้เต๋าเดียวกันต้องหยุดเป็น conflict ที่มองเห็นได้แทน last-write-wins; Production baseline ปัจจุบันยังไม่ถือว่ารับรอง contract นี้ครบถ้วน.
...
- Download ZIP ต้อง deduplicate ตาม immutable `original_storage_key` ก่อนอ่าน derivative และสร้าง archive; references ซ้ำของรูปเดียวกันเป็นเพียง read-model duplication ไม่ใช่ไฟล์ที่ต้องดาวน์โหลดซ้ำ. API ใช้ derivative ที่ `ready` เท่านั้น, แบ่ง ZIP เมื่อ raw bytes ต่อ part ถึง 16 MiB เพื่อจำกัด peak memory, upload ZIP เข้า private bucket ที่รองรับไฟล์สูงสุด 50 MiB แล้วคืน signed URL แบบ `private, no-store`; หากสร้าง ZIP ล้มเหลวต้อง cleanup artifact ที่สร้างไว้และ log เฉพาะ stage/platform trace id/error โดยไม่เปิดเผย token หรือ cookie
- `ticket.imageNames` คือ read model ที่รวมรูปรถและรูปของทุก line/เต๋าตามลำดับเดิม จึงไม่เพิ่ม API, query, storage field หรือการประกอบ fallback รายหน้า; ปุ่ม `ดูรูป` ของเต๋าเปิดลำดับรูปของทุกเต๋าตามตาราง โดยเริ่มจากเต๋าที่กดและเลื่อนไปเต๋าถัดไปได้ต่อเนื่อง ส่วนแถวซื้อเพิ่มจากสิ่งเจือปนยังเปิดเฉพาะรูปของแถวนั้น. รูปที่ยัง `queued/processing/failed` ไม่ถูกแทนด้วย original และไม่ถูกเงียบหายจาก response
- Upload evidence เป็น original-only จาก browser; server ตรวจ metadata/pixel limit ก่อนคืน reference ที่แนบได้ แล้วจึงสร้าง thumbnail แบบ background. Preview เป็น private signed thumbnail เท่านั้นและ API เป็น `private, no-store`; cron รับผิดชอบ stale lease/retry และลบเฉพาะ ledger ที่ยังไม่ผูก ticket หลัง retention จาก `system_settings`.
...
Upload/thumbnail lifecycle: browser อัปโหลด original เท่านั้นและใช้ local object URL แสดงภาพในฟอร์ม. Server ตรวจ metadata/pixel limit ก่อนสร้าง reference ที่แนบได้ บันทึก asset/job แล้วคืนผลทันทีโดยไม่รอ thumbnail และสร้าง thumbnail ผ่าน `after()`. Detail preview โหลดเฉพาะ signed thumbnail ที่ `ready`, poll รูป `queued/processing` และ retry เฉพาะ asset ของใบที่กำลังเปิด; ไม่มี fallback ไปใช้ original แทน thumbnail. ไฟล์ pending ที่หมดอายุถูก cleanup แบบ bounded หลัง upload ครั้งถัดไป โดยใช้ lock และ retention จาก `system_settings`; ไม่มี Cron รายชั่วโมง. Dimension, WebP quality, source-pixel guard, retry, polling, signed URL TTL, orphan retention, batch size และ upload concurrency อ่านจาก `system_settings`; bucket ก็ต้องตั้งใน `system_settings` และไม่มี runtime fallback. Source of truth คือ DB asset ledger + private object Storage; API ใช้ `private, no-store` และ immutable object cache key.
...
- `PUT /api/daily/weight-tickets/[id]`
  - must accept optional collaboration baseline metadata from the editor snapshot, including immutable line IDs/versions, explicit deleted line IDs, and changed header fields
...
- การลบเต๋าแรกทำได้เมื่อ section เดียวกันมีเต๋าอย่างน้อย 2 เต๋า โดยระบบเลื่อนเต๋าถัดไปเป็น line หลักและย้ายความสัมพันธ์สิ่งเจือปน/ซื้อเพิ่มตามไปด้วย; ถ้ามีเต๋าเดียวให้ลบผ่านการลบสินค้าแทน
