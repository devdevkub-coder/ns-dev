...
Upload/thumbnail lifecycle: browser อัปโหลด original เท่านั้นและใช้ local object URL แสดงภาพในฟอร์ม. Server ตรวจ metadata/pixel limit ก่อนสร้าง reference ที่แนบได้ บันทึก asset/job แล้วคืนผลทันทีโดยไม่รอ thumbnail. Detail preview โหลดเฉพาะ signed thumbnail ที่ `ready`, poll รูป `queued/processing` และโหลด original เมื่อผู้ใช้เปิดรูปเต็มเท่านั้น. Dimension, WebP quality, source-pixel guard, retry, polling, signed URL TTL, orphan retention และ upload concurrency อ่านจาก `system_settings`; bucket ก็ต้องตั้งใน `system_settings` และไม่มี runtime fallback. Source of truth คือ DB asset ledger + private object Storage; API ใช้ `private, no-store` และ immutable object cache key. Cron รับผิดชอบ stale lease/retry และ cleanup เฉพาะ unattached asset.
