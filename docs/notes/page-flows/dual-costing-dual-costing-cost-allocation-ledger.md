...
- 2026-08-04 match grouping: the main ledger now renders one summary row per `matchId`, aggregates quantities and financial facts from its lot/source rows, and provides a `ดูรายการ` dropdown for every child row. Child rows retain the verified status and reverse/reallocate actions; API, export, source-of-truth facts, and database behavior are unchanged. This keeps one allocation decision together without hiding lot-level audit detail.
