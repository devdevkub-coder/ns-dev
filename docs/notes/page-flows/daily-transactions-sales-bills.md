...
- cancel write path exists for WTO-backed Stock SB: blocks active RCP, restores consumed WTO `pending_out`, writes `SB-CANCEL`, appends `released_from_sales_bill`, returns WTO to `delivered`, reverses PO Sell usage, and appends `sales_bill_status_logs`
- Runtime cleanup removed intermediate stock-issue create/cancel entry points: new Sales Bill writes reject PSALE fields and require WTO direct for stock sale
