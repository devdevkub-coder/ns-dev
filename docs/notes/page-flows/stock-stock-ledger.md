...
- UI filter alignment 2026-07-23: desktop filter ใช้ 2 แถวแบบหน้าบิลซื้อ โดยแถวแรกเป็นค้นหา/ช่วงเวลา/สาขา และแถวที่สองเป็นสินค้า/ประเภท movement/ประเภทคลัง/การกระทำ; เหตุผลคือเงื่อนไข scope ที่ใช้บ่อยอยู่ด้านบน ส่วน filter รองและ export อยู่แถวถัดไป ทำให้ panel สั้นและ scan ได้เร็วขึ้น โดย mobile ยังคงใช้ search กับ bottom-sheet filter เดิม
- Movement direction filter 2026-07-23: `ประเภท` ให้เลือกเฉพาะ `ทุกประเภท`, `เข้า`, `ออก`; ค่า `เข้า` กรอง `qty_in > 0` และ `ออก` กรอง `qty_out > 0` เพื่อให้ผู้ใช้เลือกทิศทางธุรกิจโดยไม่ต้องรู้รหัส movement ย่อย ขณะที่รายละเอียด movement จริงยังคงแสดงใน ledger และ detail
- Product display 2026-07-23: product picker แสดงรหัสและชื่อในบรรทัดเดียว (`SKU - ชื่อสินค้า`) โดยไม่แสดงรหัสซ้ำใน description; ตาราง ledger แสดงชื่อสินค้าในบรรทัดแรกและรหัสสินค้าในบรรทัดที่สอง เพื่อให้ชื่ออ่านง่ายและยังตรวจสอบรหัสได้ทันที
- Pagination layout 2026-07-23: pagination ของ desktop และ mobile เป็นแถวอิสระระหว่าง filter กับ data surface; table card มีเฉพาะตาราง เพื่อให้ summary และการเปลี่ยนหน้าไม่ถูกมองเป็นส่วนหนึ่งของกรอบข้อมูล และสอดคล้องกับหน้าบิลซื้อ
- Location display 2026-07-23: ถ้าชื่อคลังมีชื่อสาขาอยู่แล้ว เช่น `WIP สมุทรสาคร` ให้แสดงชื่อสาขาเพียงครั้งเดียวใน list/card; ถ้าชื่อคลังไม่มีชื่อสาขา ให้แสดงคลังและสาขาแยกบรรทัดตามเดิม เพื่อไม่ปกปิดความต่างของ location ที่เป็นข้อมูลจริง
- Movement type audit 2026-07-24: ตรวจ distinct `stock_ledger.movement_type` ใน dev พบ 21 ประเภท; ทิศทางทุกประเภทสอดคล้องกับ `qty_in/qty_out` ไม่มีข้อมูลเข้า/ออกผิดทิศทางจากชุดข้อมูลที่ตรวจ ปัญหาที่พบคือ label mapping ขาดกลุ่ม production return/reverse และ reversal ทำให้ UI แสดง enum ดิบ จึงต้องให้ทุก movement type ที่ active app เขียนมี label ผ่าน `stockMovementTypeLabel`
