...
เริ่มเปิดใช้เฉพาะ WTI แล้ว: schema เพิ่ม `draft_version` ระดับเอกสาร/รายการและตาราง idempotency ของ operation, server ใช้ transaction + row version conflict, และ client ใช้ `/lines` operation route. ฟอร์มใหม่จะแสดงเฉพาะหัวเอกสารก่อน; ปุ่ม `เพิ่มรายการสินค้า` จะ validate หัวเอกสารและเปิด section สินค้าและน้ำหนัก แต่ยังไม่สร้าง draft จนกว่าจะเลือกและเพิ่มสินค้าจริง. ก่อน append สินค้าหรือเต๋ารายการใหม่ Server ต้อง validate line เดิมทั้งหมดให้ครบก่อนเสมอ. หลัง commit จะ broadcast event ที่มีเฉพาะ document/line version และให้ client ดึงเอกสารล่าสุดจาก API; Database เป็น source of truth และ realtime เป็นเพียงสัญญาณแจ้งเตือน.
...
1. ซ่อน section `สินค้าและน้ำหนัก` สำหรับ WTI ใหม่
2. เมื่อข้อมูลหัวเอกสารครบ ผู้ใช้กด `เพิ่มรายการสินค้า` เพื่อเปิด section
3. การเปิด section ยังไม่สร้าง draft
4. เมื่อเลือกสินค้าและเพิ่มรายการจริง จึงสร้าง WTI และ line แรกเป็น `draft` ใน transaction เดียวกัน
5. ก่อนเพิ่มสินค้า/เต๋ารายการถัดไป ต้อง validate line เดิมทั้งหมดตาม validation ของเต๋าที่ใช้อยู่
6. หากมี line เดิมไม่ครบ ให้ปิดการเพิ่มรายการใหม่, focus รายการที่ผิด และรอให้แก้จนผ่าน validation

รายการที่ยังไม่ครบสามารถคงอยู่ใน draft ได้ แต่ห้ามสร้างรายการใหม่ต่อจากรายการนั้น เพื่อป้องกัน validation หลุด

### หลักการบันทึก

เมื่อมีการเลือกสินค้าและเพิ่มรายการครั้งแรก ระบบสร้างเอกสารเป็น `draft` อัตโนมัติ โดย auto-save เฉพาะการเพิ่มสินค้า/เต๋า และการเพิ่ม/ลบรูปหรือสิ่งเจือปน. การแก้ไขข้อมูลเดิม เช่น น้ำหนัก รายละเอียด หรือการลบ line สินค้าหลัก ต้องกด `บันทึก`:
...
เฉพาะ operation ที่ระบุว่า auto-save เท่านั้นที่ถือว่าเป็นข้อมูลใน draft ทันที. ปุ่ม `ยกเลิก` จะไม่ย้อนกลับรายการที่ auto-save ไปแล้ว แต่ถ้ามี manual changes ในหัวเอกสารหรือข้อมูลรายการที่ยังไม่กด `บันทึก` ต้องถามยืนยันก่อนออกจากฟอร์ม
...
การรับ realtime จากผู้ใช้อื่นต้องไม่ข้าม validation gate; หากมี line ที่ไม่ครบ ปุ่มเพิ่มสินค้า/เพิ่มเต๋าต้องถูกปิดจนกว่าข้อมูลล่าสุดจะผ่าน validation
...
4. แยก auto-save เฉพาะการเพิ่มสินค้า/เต๋า และการเพิ่ม/ลบรูป/สิ่งเจือปนออกจาก manual-save fields
...
- [ ] `WTI-00` สรุป field ownership ของฟอร์ม: auto-save เฉพาะการเพิ่มสินค้า/เต๋า และการเพิ่ม/ลบรูป/สิ่งเจือปน; การแก้ไขข้อมูลเดิมใช้ manual save
- [ ] `WTI-00A` เพิ่ม flow gate ซ่อน section สินค้าและน้ำหนักจนกด `เพิ่มรายการสินค้า` หลังหัวเอกสารครบ
...
- [ ] `WTI-21` เพิ่ม endpoint append line สำหรับ `เพิ่มสินค้า` และ `เพิ่มเต๋า` พร้อม validation gate ของ line เดิม
...
- [ ] `WTI-41` auto-save เฉพาะการเพิ่มสินค้า/เต๋า และการเพิ่ม/ลบรูป/สิ่งเจือปน; การแก้ไขข้อมูลเดิมและการลบ line หลักต้องกด `บันทึก`
- [ ] `WTI-41A` ปิดปุ่มเพิ่มรายการและแสดง error/focus เมื่อมี line เดิมไม่ผ่าน validation
...
- [ ] `WTI-51A` ทดสอบการปิดกั้นเพิ่มสินค้า/เต๋าเมื่อ line เดิมไม่ครบ รวมถึง line ที่ไม่ครบจาก realtime
