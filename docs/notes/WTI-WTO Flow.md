...
- `auto_save`: เมื่อมีรายการเดิมที่กรอกครบแล้ว ผู้ใช้กดเพิ่มเต๋าหรือเพิ่มรายการต่อ ระบบบันทึกเอกสารเป็น `draft` ก่อนเพิ่มแถวถัดไป จากนั้นจึงเปิดตัวเลือกสินค้า/เต๋าในหน้าจอเดิม; การกดเพิ่มสินค้าครั้งแรกที่ยังไม่มี line จะสร้างแถวว่างใน client state โดยไม่ยิง save
...
เหตุผล: payload ที่ไม่มี line ยังไม่ผ่าน contract บันทึกจริง จึงไม่ควรยิง save ตอนเปิดแถวแรก. เมื่อมี line ที่ครบแล้วจึง persist draft เพื่อให้การเพิ่มรายการต่อเนื่องใช้ `ticket_id` เดิม และ final save update เอกสารเดียวกัน. WTI ยังคงใช้ contract เดิมที่อนุญาตให้บันทึกโดยไม่มีรายการสินค้า.
...
เมื่อผู้ใช้กด `เพิ่มสินค้า` ครั้งแรก ระบบจะสร้างแถวว่างใน client state ก่อน เพราะยังไม่มี
line payload ที่ถูกต้องสำหรับบันทึกเอกสาร. เมื่อมีรายการเดิมที่กรอกครบแล้วและผู้ใช้กด
`เพิ่มเต๋า` ระบบจึงบันทึก draft ผ่าน contract เดิมก่อนเพิ่มแถวถัดไป เพื่อไม่ให้ข้อมูลที่
กรอกครบแล้วสูญหาย. การบันทึก draft ไม่ยืนยันเอกสารและไม่สร้างการจอง `pending_out`;
การยืนยัน WTO ยังคงเป็นจุดที่ตรวจ stock และสร้าง reservation ตาม flow เดิม.
