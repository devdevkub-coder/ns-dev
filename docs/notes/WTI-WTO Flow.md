...
การ submit แบ่งเป็นสามขอบเขตที่ไม่เขียนทับกัน:

- `header`: บันทึกสาขา/คู่ค้า/ทะเบียนรถ/รูปรถ/โกดัง/หมายเหตุผ่าน header endpoint แยก;
  เมื่อมีรายการแล้ว การเปลี่ยนสาขาหรือคู่ค้าต้องใช้ final document save เพราะมีผลต่อ
  warehouse และ party snapshot.
- `product`: บันทึกเฉพาะ parent product line ของ section นั้น.
- `lot`: บันทึกเฉพาะเต๋าที่เลือก พร้อม line สิ่งเจือปนและรายการซื้อเพิ่มที่เป็นลูกของเต๋า.

ขอบเขต line ใช้ `saveScope=section`, `sectionKind` และ `sectionLineIds`; server ตรวจ
โครงสร้าง parent, version และรายการที่ลบภายใต้ advisory lock ก่อน merge ใน transaction.
รายการของผู้ใช้อื่นที่อยู่นอก scope จะไม่ถูกส่งและไม่ถูกเขียน. สรุปน้ำหนัก/stock ของเอกสาร
ยังคำนวณจากข้อมูลทั้งใบหลังการเปลี่ยนแปลง จึงให้คนหนึ่ง submit เต๋าสินค้า A ขณะที่อีกคน
แก้เต๋าสินค้า B ได้ โดยไม่ใช้ response ของ scope A มาแทนที่ draft ของ B.

การ merge ต้องเรียงรายการเดิมตาม `line_no` และคงเลขเดิมของรายการที่ยังอยู่; รายการใหม่เท่านั้น
จึงได้เลขต่อท้าย เพื่อไม่ให้การ submit เต๋าหนึ่งทำให้ parent/impurity หรือประวัติ stock ของ
เต๋าอื่นเปลี่ยนตัวตน. สำหรับ WTO ระบบล็อก stock bucket แบบเรียงลำดับ
`branch/warehouse/product` ก่อนตรวจ stock และสร้าง pending-out เพื่อให้เอกสารคนละใบที่ใช้
stock เดียวกันไม่ตรวจผ่านพร้อมกันจนเกิดการจองเกิน.
