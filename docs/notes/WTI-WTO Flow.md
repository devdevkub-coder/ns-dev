...
- สถานะ thumbnail คือ `queued -> processing -> ready` หรือ `failed`; worker claim เป็น lease แบบ atomic และ durable cron จะ drain ทั้ง queued ที่ถึงเวลาและ processing ที่ lease หมดอายุ. Detail modal poll ตามช่วงเวลาจาก config และแสดงรูปที่พร้อมทยอยขึ้นมา. ระหว่างรอแสดงสถานะตรงไปตรงมา และห้าม fallback ไปโหลด original ในช่อง thumbnail
...
- รูปที่ยังไม่ผูก ticket (`attached_ticket_id is null`) มีสิทธิ์ cleanup หลัง retention โดย cron จะ claim lease, ลบ original และ thumbnail แล้วจึงลบ ledger row; ถ้าลบ Storage บางส่วนล้มเหลวจะคง row พร้อม error เพื่อ retry และไม่ลบ asset ที่ผูกเอกสารแล้ว. `audit:weight-ticket-image-assets` ต้องผ่านโดยไม่มี malformed/legacy reference, missing original, ready-without-thumbnail หรือ orphan ledger ก่อน SIT rollout.
...
PDF ที่ `notifyWeightTicketLine` สร้างและอัปโหลดขึ้น Storage ต้องเริ่มด้วยเอกสารธุรกิจสำหรับพิมพ์: header บริษัท, ข้อมูลคู่ค้า, รายการน้ำหนัก, สรุป, ลายเซ็น และ footer ต้องอยู่ใน A4 ตาม template ใบพิมพ์เดียวกับระบบหน้า list/detail. ส่วนใบพิมพ์หลักสร้างหน้า `1..N` ตามจำนวนรายการจริง: หน้าก่อนสุดท้ายใช้ footer ตารางว่างและกรอบสรุป 3 กรอบที่มีหัวข้อ `สรุปตามหมวดสินค้า`, `หมายเหตุ`, `ข้อมูลน้ำหนัก / Weight Info` พร้อมค่า placeholder `-` และ `( มีต่อหน้า X / Continued on Page X ➔ )` แทนลายเซ็น โดยไม่ใส่ยอดจริงระหว่างทาง; หน้าหลักสุดท้ายเท่านั้นมีสรุป หมายเหตุ และลายเซ็นจริง. หลังจากหน้าหลักทั้งหมดจึงต่อ photo album จากรูปหลักฐานของเอกสารเดียวกัน เพื่อให้ไฟล์ PDF เดียวเปิดดูได้ครบทั้งใบชั่งและรูปหน้างาน. LINE ยังสามารถส่ง album/image path แยกได้เพื่อให้คนในแชทเห็นรูปทันที.
