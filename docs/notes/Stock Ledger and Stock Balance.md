...
| `POST/PATCH /api/purchase/bills` | ตรง target หลักสำหรับ PB Stock ledger | create เขียน `PB`; cancel append `PB-CANCEL`; supplier change และ edit append `PB-EDIT-REV` แล้ว append `PB` state ใหม่ภายใต้ PB เดิม โดยไม่ delete/rebuild ledger เดิม |
