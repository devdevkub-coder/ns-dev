...
- ปุ่ม `ยกเลิก` ยัง disabled เพราะ `receipt_vouchers` ยังไม่มี status/cancel field
...
- API มี `GET`, `POST`, `PATCH`; ยังไม่มี `DELETE/CANCEL` หรือ detail route
...
| `rows[]` | array | `receipt_vouchers` | list + print snapshot ของ RV ที่บันทึกแล้ว |
...
| `sellerName` | string | yes | ผู้รับเงิน; ปัจจุบัน client ส่งจาก PB snapshot |
| `sellerTaxId` | string | no | จาก PB snapshot; blank ได้ถ้า PB snapshot blank |
| `sellerAddress` | string | no | จาก PB snapshot; blank ได้ถ้า PB snapshot blank |
| `sellerPhone` | string | no | จาก PB snapshot; blank ได้ถ้า PB snapshot blank |
| `salesPerson` | string | no | จาก PB snapshot |
| `licensePlate` | string | no | จาก PB snapshot |
| `items[]` | array | yes | อย่างน้อย 1 รายการ |
| `items[].description` | string | yes | รายการสินค้า |
| `items[].qty` | number | yes | ต้องไม่ติดลบ |
| `items[].unit` | string | no | ถ้าไม่ส่ง server ใช้ `กก.` |
| `items[].price` | number | yes | ต้องไม่ติดลบ |
...
- set `receiver_signer_name = sellerName`
...
- recompute items, totals, amount text, and cash payment method the same as create
...
| `PATCH action=cancel` or `POST /cancel` | not implemented | needs `status`, cancel reason, and RV status/timeline log columns |
| `GET /api/purchase/receipt-vouchers/[docNo]` | not implemented | current page loads list and print snapshot from collection endpoint |
...
- [ ] Add cancel route with status log/audit
