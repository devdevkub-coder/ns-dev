...
Superseding decision on 2026-05-30:
- `PMA` ที่ `approved` แล้วถือเป็น final snapshot ของการอนุมัติ ห้ามย้อนกลับเป็น `ยกเลิกแล้ว`
- คำว่า `ยกเลิก` ใช้ได้เฉพาะก่อนกดอนุมัติใน modal `/daily/payment-approval` เท่านั้น และเป็นการทิ้ง draft split ไม่ใช่การเปลี่ยนสถานะ `PMA`
- หน้า `/purchase/payments` ต้องถูกมองเป็น `รายการรอออก PMT` ไม่ใช่ queue ที่ rollback `PMA`
- สถานะ `ยกเลิกแล้ว` ต้องไปอยู่กับ `PMT` หรือ payment-voucher lifecycle เท่านั้น
...
| อนุมัติแล้ว / รอออก PMT | `approved` 1..n rows | ไม่มี หรือมี `PMT pending` | ไม่ได้ | ไม่ได้ | `/purchase/payments` | บิลถูก lock แล้ว และอาจมีหลาย approval items ต่อ 1 เอกสาร |
...
| ยกเลิกการจ่ายเงิน | `approved` ถ้ายังเหลือยอดค้าง, `paid` ถ้าถูก replace ด้วย PMT ใหม่ภายหลัง | มี `PMT cancelled` | ไม่ได้ | ไม่ได้ | `/purchase/payment-history` = `ยกเลิกแล้ว`, และถ้ายังเหลือยอดค้างต้องเห็นต่อใน `/purchase/payments` | ยกเลิกที่ PMT ไม่ใช่ rollback PMA |
