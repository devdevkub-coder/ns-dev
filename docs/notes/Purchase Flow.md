...
| 2 | จ่ายเงินล่วงหน้า / มัดจำตาม PO (optional) | Supplier, สาขา, วันที่จ่าย, วิธีจ่าย, บัญชีที่จ่าย, ยอดจ่ายล่วงหน้า, เอกสารอ้างอิง PO/ใบชั่งน้ำหนักใหญ่ถ้ามี | `ADV...` | ADV = `บันทึกแล้ว` | เป็น source แยกที่ต้องเข้า `อนุมัติจ่ายเงิน -> จ่ายเงิน Supplier` ก่อนนำไปหักกับ PB ภายหลัง |
...
| 7 | Allocate ใบรับของเข้ากับ PO/Spot | เลือก PO เพื่อเอาจำนวน/น้ำหนักไปตัด PO; ถ้าจะออกบิลเพิ่มในสินค้าเดิมให้กด `เพิ่มสินค้า` ใต้ summary เดิม แล้วเลือก `PO` อื่นหรือ `Spot Buy` | ไม่มี | Draft PB | แยกยอดใบรับของเดียวกันเป็น PO และ Spot ได้ และบันทึกแบบ partial ได้ |
...
| 11 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `ออกบิลแล้วบางส่วน` หรือ `เสร็จสิ้น` | กันการนำใบรับของไปออกบิลซ้ำเกินยอด และยังเลือกต่อได้ถ้ายังเหลือ |
...
| 14 | อนุมัติจ่ายเงินยอดคงเหลือ | เลือกบิลซื้อที่ยังค้างหลังหัก ADV, split วิธีจ่าย/บัญชีปลายทางตามที่ต้องการ, กรอกยอดอนุมัติแต่ละ split, หมายเหตุ | `PMA{branchCode}{YYMM}-NNNN` | Approval = `อนุมัติแล้ว` | สร้าง approval snapshot เฉพาะยอดคงเหลือ; ถ้า ADV ตัดเต็มไม่ต้องเข้า queue นี้ |
| 15 | จ่ายเงิน Supplier | เลือกรายการอนุมัติที่ยังไม่จ่าย, บัญชีจ่าย, วิธีจ่าย, ยอดโอนจริง, WHT/ส่วนลด, หมายเหตุ | `PMT...` | Payment = `บันทึกจ่ายแล้ว` | บันทึกเงินออกและตัดเจ้าหนี้จาก approval snapshot |
| 16 | ระบบอัปเดตบิลรับซื้อ | ไม่ต้องกรอกเพิ่ม | ไม่มี | PB = `จ่ายบางส่วน` หรือ `จ่ายครบ` | ยอดค้างจ่ายลดลงจาก payment rows ที่ผูก approval และ ADV allocation |
| 17 | ยกเลิกบิลรับซื้อ (ถ้ายังไม่มีการจ่าย) | กรอกเหตุผลการยกเลิก | ไม่มี | PB = `ยกเลิก` | ลบผลกระทบ stock ledger ของบิล, recalc การใช้งาน WTI ใหม่จากบิลที่ยัง active อยู่, และคืนยอด ADV allocation ให้ใช้ได้ตาม policy |
...
- เมื่อ `ยกเลิกการจ่ายเงิน` สำเร็จ:
  ...
  - PMT เดิมต้องอยู่ใน history เป็น `ยกเลิกแล้ว` พร้อมเหตุผล ผู้ยกเลิก และเวลา
  - PMA เดิมถือว่าจบ cycle แล้ว ใช้เป็น audit/history เท่านั้น ไม่ reuse เพื่อจ่ายใหม่
  - source document ต้องกลับไป `/daily/payment-approval` เป็น `ยังไม่อนุมัติ`
  - ถ้าจะจ่ายใหม่ ต้องอนุมัติใหม่และสร้าง `PMA` ใหม่ก่อนออก `PMT` ใหม่
  - หลังจาก source กลับไป `ยังไม่อนุมัติ` และไม่มี active PMT/PMA cycle แล้ว บิลซื้อจึงจะยกเลิกได้ตาม guard
...
- ถ้ามีการ `ยกเลิกการจ่ายเงิน` แล้วกลับไป `อนุมัติใหม่ -> จ่ายใหม่` ต้องเกิด `PMA` ใหม่และ voucher/payment row ใหม่ ไม่ overwrite PMA/PMT เดิม
...
| ยกเลิกการจ่ายเงิน | PMA เดิมจบ cycle และใช้เป็น audit/history เท่านั้น; ถ้าจะจ่ายใหม่ต้องสร้าง PMA ใหม่ | มี `PMT cancelled` | ได้หลัง source กลับไป `ยังไม่อนุมัติ` และไม่มี active cycle | ได้ตาม guard หลัง source กลับไป `ยังไม่อนุมัติ` | `/purchase/payment-history` = `ยกเลิกแล้ว`, `/daily/payment-approval` = `ยังไม่อนุมัติ` | ยกเลิกที่ PMT เกิดหลังมี payment แล้ว ถือว่าเริ่ม payment cycle ใหม่ |
...
5. กดยกเลิก `PMT`:
   - เป็นการยกเลิก payment execution/voucher และปิด payment cycle เดิม
   - history ต้องเห็นรายการนั้นเป็น `ยกเลิกแล้ว`
   - PMA เดิมไม่ถูกนำกลับไปใช้จ่ายซ้ำ
   - source document ต้องกลับไป `/daily/payment-approval` เพื่ออนุมัติใหม่
   - ถ้าจะจ่ายใหม่ ต้องสร้าง `PMA` ใหม่ แล้วจึงสร้าง `PMT` ใหม่
...
-> PMA-... อนุมัติยอดคงเหลือหลังหัก ADV
-> PMT012605-0001 บันทึกจ่ายยอดคงเหลือ
...
-> WTI012605-0001 รับของแล้ว
-> PB012605-0001 เปิดอยู่ (เลือก WTI, allocate ไป PO บางส่วน, และหัก ADV บางส่วนถ้ามี)
...
| 2 | จ่ายเงินล่วงหน้า / มัดจำตาม PO (optional) | Supplier, สาขา, วันที่จ่าย, วิธีจ่าย, บัญชีที่จ่าย, ยอดจ่ายล่วงหน้า, เอกสารอ้างอิง PO/ดีล | `ADV...` | ADV = `บันทึกแล้ว` | เป็น source แยกที่ต้องเข้า `อนุมัติจ่ายเงิน -> จ่ายเงิน Supplier` ก่อนนำไปหักกับ PB Trading ภายหลัง |
...
| 7 | อนุมัติจ่ายเงินยอดคงเหลือ | เลือกยอดค้างหลังหัก ADV, split วิธีจ่าย/บัญชีปลายทางตามต้องการ | `PMA...` | Approval = `อนุมัติแล้ว` | ถ้า ADV ตัดเต็มไม่ต้องเข้า queue นี้ |
| 8 | จ่ายเงิน Supplier | เลือกรายการอนุมัติที่ยังไม่จ่าย, บัญชีจ่าย, วิธีจ่าย, ยอดจ่าย, WHT/ส่วนลด, หมายเหตุ | `PMT...` | Payment = `บันทึกจ่ายแล้ว` | บันทึกเงินออกและตัดเจ้าหนี้ |
| 9 | ระบบอัปเดตบิลรับซื้อ | ไม่ต้องกรอกเพิ่ม | ไม่มี | PB = `จ่ายบางส่วน` หรือ `จ่ายครบ` | ยอดค้างจ่ายลดลงจาก payment rows และ ADV allocation |
...
| 3 | อนุมัติและจ่าย ADV (ถ้ามี) | `รายการประจำวัน > อนุมัติจ่ายเงิน`, แล้ว `รายการประจำวัน > จ่ายเงิน Supplier` | `/daily/payment-approval`, `/purchase/payments` |
...
| 10 | อนุมัติและจ่ายยอดคงเหลือหลังหัก ADV | `รายการประจำวัน > อนุมัติจ่ายเงิน`, แล้ว `รายการประจำวัน > จ่ายเงิน Supplier` | `/daily/payment-approval`, `/purchase/payments` |
| 11 | ระบบอัปเดตบิลรับซื้อ | ระบบอัตโนมัติ | - |
...
| 8 | อนุมัติและจ่ายยอดคงเหลือหลังหัก ADV | `รายการประจำวัน > อนุมัติจ่ายเงิน`, แล้ว `รายการประจำวัน > จ่ายเงิน Supplier` | `/daily/payment-approval`, `/purchase/payments` |
