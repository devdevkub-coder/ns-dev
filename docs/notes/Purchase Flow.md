...
| 5 | Allocate ใบรับของเข้ากับ PO/Spot | เลือก PO เพื่อเอาจำนวน/น้ำหนักไปตัด PO; ถ้าใบรับของเกิน PO ให้กด `เพิ่มสินค้า` เลือกใบรับของเดิม แล้วเลือก `Spot Buy` สำหรับยอดส่วนเกิน | ไม่มี | Draft PB | แยกยอดใบรับของเดียวกันเป็น PO และ Spot ได้จนยอดครบใบรับของ |
| 6 | บันทึกบิลรับซื้อ | กรอกราคา, VAT, เลขอ้างอิง Supplier, หมายเหตุ และตรวจยอด allocation | `PB...` | PB = `เปิดอยู่` | ตั้งเจ้าหนี้/AP |
| 7 | ระบบอัปเดตใบรับของ | ไม่ต้องกรอกเพิ่ม | ไม่มี | ใบรับของ = `ออกบิลบางส่วน` หรือ `ออกบิลแล้ว` | กันการนำใบรับของไปออกบิลซ้ำเกินยอด |
| 8 | ระบบอัปเดต PO จากบิล | ไม่ต้องกรอกเพิ่ม | ไม่มี | PO = `ออกบิลบางส่วน` หรือ `ออกบิลแล้ว` | ตัด PO เฉพาะยอดที่ allocate เป็น PO |
| 9 | ระบบลง stock ถ้าเป็นบิลเข้าสต๊อก | ไม่ต้องกรอกเพิ่ม | อาจเกิด stock ledger id ภายใน | Stock = `บันทึกแล้ว` | เพิ่ม stock และต้นทุน; ถ้าเป็นทองแดง/ทองเหลืองให้เข้า Cost Pool |
| 10 | จ่ายเงิน Supplier | เลือกบิลค้างจ่าย, บัญชีจ่าย, วิธีจ่าย, ยอดจ่าย, WHT/ส่วนลด, หมายเหตุ | `PMT...` | Payment = `บันทึกจ่ายแล้ว` | บันทึกเงินออกและตัดเจ้าหนี้ |
| 11 | ระบบอัปเดตบิลรับซื้อ | ไม่ต้องกรอกเพิ่ม | ไม่มี | PB = `จ่ายบางส่วน` หรือ `จ่ายครบ` | ยอดค้างจ่ายลดลง |
