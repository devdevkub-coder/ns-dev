...
- `allocationRows` และ `editItems` เป็น current-source read model: ข้อมูล WTI/PO ปัจจุบันต้องอ่านจาก allocation ที่มี `allocation_status = active` เท่านั้น; เมื่อ allocation ถูก release แล้วห้าม fallback ไป `purchase_bill_items.po_buy_id`, relation `item.po_buys` หรือ `source_snapshot` เพื่อเติมค่าใน field ปัจจุบัน เพราะจะทำให้ source เก่ากลับมาเป็น source ของ version ใหม่
- หากหน้าประวัติหรือเอกสารพิมพ์ต้องแสดง source เดิมหลัง release ให้ใช้ historical projection/snapshot ที่ระบุชัดว่าเป็นประวัติ แยกจาก current-source fields; สำหรับ supplier change ระบบ release PO เดิมและสร้างชุด Spot Buy ใหม่ จึงไม่อ้าง PO เดิมเป็น active source ของ version ใหม่
