...
### CACHE-M7 WTI/WTO Print Derivative Contract (2026-08-20)

WTI/WTO เป็นข้อมูลภายในระดับ L5 และ source of truth ยังคงเป็น original ใน private image bucket กับ ledger row `weight_ticket_image_assets`; binary ไม่อยู่ใน Redis, browser memory หรือ option response. `thumbnail` ใช้สำหรับ gallery/detail preview ที่ bounded ตาม contract เดิม ส่วน `print` เป็น JPEG derivative แยกสำหรับ browser print, React-PDF และ LINE โดยใช้ immutable key, `fit: inside`, `withoutEnlargement: true`, `rotate()` และกรอบสูงสุด `400 × 400 px` ที่ quality `90`. รูปที่อัปโหลดใหม่ใช้ sibling key จาก UUID เดิม เช่น `.print.jpg`; แถวเก่าที่ migration เติม key ใช้ namespace `attachments/migrated-print/{assetId}.print.jpg` เพื่อไม่เดา/ทับ object เดิม. ภาพแนวตั้งจึงได้ไม่เกิน `300 × 400` โดยประมาณ และภาพแนวนอนได้ไม่เกิน `400 × 300` โดยไม่ยืดหรือ crop ratio. Browser prefetch อนุญาตเฉพาะ in-flight dedup และการ warm signed image ผ่าน HTTP cache; ห้ามเก็บ `WeightTicketRecord` เต็มไว้ใน browser memory และ formal print จะ retry เฉพาะ readiness error แบบมีขอบเขต.

#### What is what / why

- User เลือกรูป -> upload original ที่ private และ immutable -> สร้าง ledger row พร้อม `thumbnail_storage_key`/`print_storage_key` และสถานะ `queued` -> worker อ่าน original แล้วสร้าง thumbnail/print derivative แยกกัน -> upload derivative แบบ `upsert: false` -> update สถานะ `ready` หลัง Storage สำเร็จเท่านั้น. ถ้า upload ตอบว่า object มีอยู่แล้ว worker/backfill ต้อง download และตรวจ byte ให้ตรงกับ derivative ที่ generate จาก original ปัจจุบันก่อนจึง mark `ready`; object ที่ byte ไม่ตรงจะถูกคงเป็น `queued`/error เพื่อป้องกันการยอมรับไฟล์ผิดตัว.
- Download รูปใช้ derivative แยกจาก Original: worker สร้าง JPEG ด้านยาวสูงสุด `1600px`, quality `88`, `fit: inside`, `withoutEnlargement: true` และเก็บ `download_storage_key`/สถานะ/ขนาดใน ledger. Download API ใช้เฉพาะ derivative ที่ `ready`; ไม่ fallback ไป Original, thumbnail, legacy URL หรือ base64. ถ้า ZIP ใหญ่ให้แบ่งเป็นหลาย ZIP artifact ไม่เกิน threshold ต่อส่วน แล้วอัปโหลดไว้ใน private image Storage ก่อนออก signed URL อายุสั้นให้ browser ดาวน์โหลด.
- UI preview เซ็นเฉพาะ thumbnail แบบ private signed URL; formal print/PDF/LINE เรียก `includePrintImages=true` หรือ strict print resolver แล้วเซ็นเฉพาะ print derivative. HTML/React-PDF/LINE builders ใช้ strict resolved-image helper และหยุดทั้งเอกสารเมื่อ stored attachment ใดไม่มี print derivative แทนการเงียบหาย. Original ใช้ได้เฉพาะ explicit full-resolution detail/download flow และไม่ถูก promote เข้า PDF/LINE.
- ถ้า ledger, derivative key, object หรือสถานะ print ไม่พร้อม ระบบหยุด formal document generation พร้อม `WEIGHT_TICKET_PRINT_IMAGE_NOT_READY` และ HTTP `409` (ข้อมูลยังไม่พร้อม) หรือ `503` (Storage/signing/config ใช้งานไม่ได้); ห้าม fallback ไป original, legacy URL, filename-only หรือ base64. ผู้ใช้ retry หลัง worker/backfill ทำให้สถานะ `ready`.
- `print_status`, attempt/lease/error และ dimensions อยู่ใน DB เพื่อให้ queued, retry, stale lock, failed และ broken/missing image ตรวจสอบได้โดยไม่ใช้ cache เป็น source of truth. Cleanup ลบ original กับ derivative ทั้งชุดเฉพาะ unattached asset ที่หมดอายุและไม่มี worker lock.

#### Cache, key, invalidation and validation

- Signed URL ของ thumbnail/print เป็น response แบบ `private, no-store` และ TTL จาก `WEIGHT_TICKET_IMAGE_PREVIEW_TTL_SECONDS`; object key เป็น versioned/immutable จึงใช้ `Cache-Control: 31536000` ได้. `ready` print ต้องมี dimensions จริงทั้งคู่และไม่เกิน `400 × 400`; audit จะ reject row ที่ dimensions หายหรือเกินกรอบ. ไม่มี Redis key ใหม่สำหรับ binary และไม่มี persistent browser cache. WTI/WTO hover/focus prefetch ดึง ticket detail ที่มี `includePrintImages=true` ด้วย in-flight dedupe และอุ่นเฉพาะ `printUrl`; ไม่เก็บ `WeightTicketRecord` เต็มไว้ใน browser memory. ตอนกดพิมพ์จะ fetch ticket ล่าสุดเสมอ และ retry เฉพาะ `WEIGHT_TICKET_PRINT_IMAGE_NOT_READY` แบบ bounded `500ms -> 1s -> 2s`; generation guard จะทิ้งผลจาก request เก่าที่เริ่มก่อน invalidate หลัง edit/cancel/confirm หรือ realtime change. ห้ามใช้ thumbnail เป็น formal print fallback.
- หลัง upload derivative สำเร็จจึง update ledger และ thumbnail/print worker ใช้ lease-owner CAS ตอน update success/failure เพื่อไม่ให้ worker ที่ถูก reclaim เขียนทับสถานะของ worker ใหม่. Backfill reclaim เฉพาะ lease ที่เกิน `WEIGHT_TICKET_THUMBNAIL_LOCK_TIMEOUT_SECONDS` และรายงาน `ready`/`nonReadyRemaining` จาก DB หลังจบรอบ; `--apply` จะ fail หากยังมีแถวที่ไม่ ready. Migration `20260820130000_add_weight_ticket_print_derivatives` และ `20260821100000_add_weight_ticket_notification_artifacts` ถูก apply/record บน SIT แล้ว และ backfill `--apply --expected-project-ref=vbjlkxbytccklhqvxjuu` สร้าง/ตรวจ print derivative ครบ 510/510 แถว (`ready=510`, `nonReadyRemaining=0`, errors=0) ด้วย settings `400/90`. Print-readiness ผ่านครบและไม่มี missing/queued/invalid/collision/duplicate print key/orphan print. Full audit ยังมี reference findings เดิม `attachedWithoutReference=7` และ `duplicateReference=16`; ต้องแยก cleanup reference data ภายหลัง โดยห้าม fallback หรือแก้ source evidence ใน batch backfill นี้. `backfill` แบบ dry-run เป็นค่าเริ่มต้น และ `--apply` เท่านั้นที่แก้ Storage/DB.
- LINE Flex ใช้ URL ของภาพอัลบั้ม outbound ที่อัปโหลดแล้วเป็น image tile/action เพื่อให้ข้อความที่อยู่นอกระบบไม่พึ่งพา private print signed URL อายุสั้น; private print derivative ใช้เป็น source ระหว่างสร้าง PDF/album เท่านั้น และต้องไม่ถูกฝังลง payload LINE โดยตรง.
- LINE notification สร้าง PDF/album ใน public outbound bucket แยกจาก private source evidence และเก็บ `{bucket, kind, storageKey}` ของ artifact ที่ส่งสำเร็จไว้ใน `weight_ticket_notification_logs.artifact_storage_keys`. ถ้าไม่มี target ใดยืนยันการรับข้อความ ระบบลบ artifact ที่สร้างใน request นั้นทันทีและไม่เก็บ key ที่ถูกลบสำเร็จไว้ใน failed/conflict log; ถ้าลบไม่สำเร็จจะคืน `LINE_ARTIFACT_CLEANUP_FAILED` และคง key ไว้ใน recovery log ให้ operator กู้คืนแบบ manual. เมื่อมี target สำเร็จอย่างน้อยหนึ่งรายจึงเก็บ artifact ไว้สำหรับลิงก์ที่ส่งออกไป. แนวทางนี้เป็น request-scoped cleanup และไม่เพิ่ม cron ใหม่.
- Focused tests ต้องครอบ WTI และ WTO, vehicle/product/line images, หลายหน้าใน formal A4 โดยคง 6 รูปต่อหน้า/รูปที่ 7 ขึ้นหน้าใหม่, LINE outbound album/Flex โดยคง 8 รูปต่อ page artifact/รูปที่ 9 ขึ้นหน้าใหม่, portrait/landscape ratio, cache/signing hit-miss, queued/failed/missing ledger, broken key, object-byte mismatch, lease reclaim และยืนยันว่า original ไม่ถูก sign/upload เพื่อ formal output.

#### CACHE-M8 On-demand PDF Download (2026-08-21)
...
- ปุ่ม `พิมพ์` ของ WTI/WTO เปิด PDF ที่สร้างจาก endpoint เดียวกับ direct download ในหน้าต่างใหม่ เพื่อให้ browser PDF viewer พิมพ์ไฟล์เดียวกันและไม่เกิด DOM pagination คนละชุด. Click-time print ไม่โหลด ticket/signed images ซ้ำใน browser; PDF endpoint จะ await request-driven image worker ก่อน strict readiness check และ helper retry เฉพาะ `WEIGHT_TICKET_PRINT_IMAGE_NOT_READY` แบบ bounded `500ms -> 1s -> 2s` สำหรับ race ที่ยังเหลือ. ปุ่มยังใช้คำว่า `พิมพ์` เท่านั้น; HTML print builder เดิมยังคงเป็นชุดทดสอบ/แหล่งอ้างอิงจนกว่าจะมีการลบอย่างปลอดภัย.
