...
- ปุ่ม `พิมพ์` ของ WTI/WTO เปิด PDF ที่สร้างจาก endpoint เดียวกับ direct download ในหน้าต่างใหม่ เพื่อให้ browser PDF viewer พิมพ์ไฟล์เดียวกันและไม่เกิด DOM pagination คนละชุด. Click-time print ไม่โหลด ticket/signed images ซ้ำใน browser; endpoint จะ request-driven kick image worker ก่อน strict readiness check และ helper retry เฉพาะ `WEIGHT_TICKET_PRINT_IMAGE_NOT_READY` แบบ bounded `500ms -> 1s -> 2s`. ปุ่มยังใช้คำว่า `พิมพ์` เท่านั้น; HTML print builder เดิมยังคงเป็นชุดทดสอบ/แหล่งอ้างอิงจนกว่าจะมีการลบอย่างปลอดภัย.
