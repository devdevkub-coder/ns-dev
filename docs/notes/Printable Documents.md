...
- เอกสารธุรกิจที่ใช้ measured A4 layout กลาง (`corporate-print-layout`) จะวัดหลังโหลด font/logo แล้ว. ความสูงจริงเป็นตัวตัดสินการแบ่งหน้า; 15 รายการเป็นจำนวนสูงสุดต่อหน้า ไม่ใช่จำนวนที่ต้องยัดให้ครบ. เอกสารทั่วไปมีหัวข้อ placeholder และ `( มีต่อหน้า X / Continued on Page X ➔ )`, ส่วนยอดจริง/หมายเหตุจริง/ลายเซ็นอยู่หน้าสุดท้ายเท่านั้น; WTI/WTO เป็นข้อยกเว้นที่ใช้พื้นที่ placeholder เดิมเพิ่มแถวและแสดงเฉพาะข้อความมีต่อหน้า.
...
| P0 | `WTI/WTO` ใบรับของ/ใบส่งของจากงานชั่ง | `/daily/weight-ticket-list` | Implemented print, share/audit follow-up | `printWeighingTicket(ticket)` และปุ่ม `ใบชั่ง` ที่ `old-apps/legacy/index.html:52560` ถึง `old-apps/legacy/index.html:52985` | Active helper รองรับ WTI/WTO แล้ว; ส่วนใบพิมพ์หลักสร้างหน้า A4 `1..N` ตามจำนวนรายการ โดยหน้าต่อเนื่องใช้พื้นที่ส่วนล่างของแบบฟอร์มเพิ่มรายการให้ได้สูงสุด 20 แถวและแสดงเฉพาะข้อความมีต่อหน้า ส่วนยอด/หมายเหตุจริง/กรอบ `สรุปตามหมวดสินค้า`, `ข้อมูลน้ำหนัก / Weight Info` และลายเซ็นอยู่หน้าสุดท้ายของใบพิมพ์หลัก จากนั้นจึงต่อด้วยหน้าอัลบั้มรูปหลักฐานจากรูปรถและรูปสินค้า |
...
- เอกสารที่ใช้ placeholder บนหน้า `1..N-1` ต้องมี footer ตารางว่างและกรอบสรุปตาม template ของเอกสาร โดยไม่แสดงยอดหรือหมายเหตุธุรกิจจริง พร้อมข้อความ `Continued on Page X` แทนช่องลงนาม เพื่อไม่ให้ยอดระหว่างทางถูกเข้าใจว่าเป็นยอดเอกสารฉบับสมบูรณ์; WTI/WTO ใช้พื้นที่กรอบเดิมเพิ่มแถวและไม่แสดงกรอบ placeholder บนหน้าต่อเนื่อง
...
- `WTI/WTO` ใช้แบบฟอร์มชั่งแบบตาราง 20 ช่องทุกหน้า รวมหน้าสุดท้ายที่มียอดรวม/กรอบสรุป/ลายเซ็น: ต้องเติมแถวว่างให้ครบ 20 ช่องเสมอ แม้ข้อมูลจริงจะมีน้อย เพื่อให้เส้นตารางและพื้นที่แบบฟอร์มต่อเนื่องกัน (ข้อมูลจริงสูงตามเนื้อหา แถวว่างแบ่งพื้นที่ที่เหลือของตารางอย่างเท่ากัน); หน้าหลัก `1..N-1` ไม่มีกรอบสรุป/หมายเหตุ/Weight Info และใช้พื้นที่เดิมของกรอบเหล่านั้นให้ตารางรายการ โดยเหลือเพียงข้อความมีต่อหน้า; หน้าหลัก `N` เท่านั้นมีสรุป/หมายเหตุ/ลายเซ็นจริง จากนั้นจึงต่ออัลบั้มหลักฐาน 6 รูปต่อหน้า
- สำหรับ WTI/WTO แถวปกติที่ไม่มีรายละเอียดและไม่พอดีกับพื้นที่หน้าปัจจุบันให้ย้ายทั้งแถวไปหน้าถัดไป; ถ้ามีรายละเอียด ให้ใช้พื้นที่ที่เหลือของหน้าปัจจุบันตัดเฉพาะข้อความเป็นแถวต่อเนื่อง (`ต่อรายละเอียด`) แล้วนำส่วนที่เหลือไปหน้าถัดไป และถ้ารายละเอียดสูงเกินหนึ่งหน้าให้แบ่งต่อเนื่องหลายหน้า โดยคงชื่อสินค้าและน้ำหนัก/ยอดรวมไว้เฉพาะแถวแรก และเว้นค่าตัวเลขในแถวต่อเนื่องเพื่อไม่ให้ยอดถูกนับซ้ำ; HTML preview และ React-PDF ต้องใช้กติกาเดียวกัน
- ปุ่มพิมพ์ WTI/WTO ต้องอ่าน document snapshot ล่าสุดอีกครั้งเมื่อผู้ใช้กดพิมพ์ เพื่อไม่ให้ cache จากการเปิด/hover ก่อนหน้าเอาข้อมูลเก่ามาสร้าง page plan; การอ่านใหม่นี้เป็น read-only และไม่เปลี่ยนข้อมูลเอกสาร
...
## 2026-08-08 Continuation Placeholder Panel Contract (Non-WTI/WTO)
...
Formal multi-page documents other than WTI/WTO keep their real totals, business notes and signatures on the final page only. Their pages `1..N-1` keep the continuation marker and titled placeholder panels instead of blank frames:
...
Each placeholder panel contains `-` as the explicit empty value. The contract applies to both the direct browser print builders and the PMT/RCP builders embedded in `MoneyMovementPageClient`; it does not change report-style screen printing or any document data, totals, signatures or persistence behavior. WTI/WTO follows the dedicated 20-row continuation contract above instead of rendering these panels on intermediate pages.
...
- WTI/WTO form pages use a dedicated 20-row logical ceiling. The browser remeasures the actual DOM after the Thai font and logo are ready, while server PDF uses the same ordered source-row plan with conservative height budgeting. A long row moves as a complete row when possible; if one row exceeds a page, only its detail text is split into continuation rows so data is never truncated or silently dropped.
- The final form page intentionally reserves the summary, totals, and signature area, so it may contain fewer than 20 rows. Intermediate form pages use the freed lower-form space for item rows and retain only `Continued on Page X` plus the preserved footer; attachment-album pages are separate from the form-row capacity. This is presentation-only and preserves the WTI/WTO snapshot, totals, API, DB, Storage, and notification contracts.
...
- Pagination parity with HTML: the PDF uses the same ordered source-row plan, so 20 short rows render as one A4 form page and 21+ rows spill to a continuation page with the continuation marker only; a normal row that does not fit moves whole, while a row with detail text uses the remaining space for a detail continuation before moving the rest to the next page. Dense styling applies only above 14 items; the former WTI 12 / WTO 14 final-page reserve is removed.
...
- `print-asset-prefetch.ts` provides shared company/font/logo asset warm-up and `invalidateWeightTicketForPrintCache(ticketId?)` for edit flows. Ticket-detail prefetch helpers remain isolated in the module but are not invoked by the print menu, because the click path must read a fresh document snapshot.
- `WeightTicketListPageClient.tsx` warms only shared print assets on `onMouseEnter`/`onFocus`; when the user clicks `พิมพ์`, `handlePrintTicket` invalidates any older ticket cache and reads the document snapshot again with image previews. It does not use a cached ticket snapshot or fallback to `includeImagePreviews=false`; preview/data errors fail closed and are shown to the user.
...
5. Tests — `weight-ticket-print.test.ts` (34 focused tests), `corporate-print-layout.test.ts`, `document-print-contract.test.ts`, and `weight-ticket-storage.test.ts` cover the shared asset warm-up and fresh-read print boundary. Ticket data and image-preview failures are not silently swallowed by the click-time print path.
