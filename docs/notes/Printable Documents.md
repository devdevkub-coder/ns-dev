...
- สำหรับ WTI/WTO แถวปกติที่ไม่พอดีกับพื้นที่หน้าปัจจุบันให้ย้ายทั้งแถวไปหน้าถัดไป แต่ถ้ารายละเอียดของแถวเดียวสูงเกินหนึ่งหน้า ให้ตัดเฉพาะข้อความรายละเอียดเป็นแถวต่อเนื่อง (`ต่อรายละเอียด`) ได้ โดยคงชื่อสินค้าและน้ำหนัก/ยอดรวมไว้เฉพาะแถวแรก และเว้นค่าตัวเลขในแถวต่อเนื่องเพื่อไม่ให้ยอดถูกนับซ้ำ; HTML preview และ React-PDF ต้องใช้กติกาเดียวกัน
...
- Pagination parity with HTML: the PDF uses the same ordered source-row plan, so 20 short rows render as one A4 form page and 21+ rows spill to a continuation page with the continuation marker only; a row that does not fit moves whole, while an oversized row splits only its detail text. Dense styling applies only above 14 items; the former WTI 12 / WTO 14 final-page reserve is removed.
