...
1. **สิทธิ์การใช้วงเงิน OD (Current Accounts Only):**
   ...
   - `FCD` เป็นคุณสมบัติของบัญชีธนาคาร ไม่ใช่ประเภทบัญชีที่ตัดสิทธิ์ OD ดังนั้น FCD แบบกระแสรายวันสามารถมีวงเงิน OD ได้ ส่วนออมทรัพย์ `savings` และบัญชีเงินสด `cash` ห้ามตั้งวงเงิน OD
   - วงเงินและการใช้ OD ของ FCD ต้องคำนวณตามสกุลเงินของ Statement ห้ามรวมกับ THB โดยไม่มีอัตราแลกเปลี่ยนและกติกาแปลงสกุลที่บันทึกไว้

2. **ยอดเงินที่จ่ายได้จริง (Available to Pay):**
   - คำนวณจาก: `Available to Pay = ยอดหลักคงเหลือ + OD คงเหลือ`
   - เมื่อยอดหลักหมด ระบบแสดงยอดหลักเป็น `0` และแยกยอดที่ใช้จาก OD ไว้ใน `OD ใช้ไป`; ไม่แสดงยอดหลักติดลบ
...
3. **การตรวจสอบและป้องกันเงินไม่พอ (Available to Pay Validation):**
   ...
   - **ฝั่งบันทึกค่าใช้จ่ายแบบจ่ายทันที (`/daily/expenses`):** ยอดสุทธิหลังส่วนลดและค่าธรรมเนียมต้องไม่เกิน Available to Pay ของบัญชีจ่าย; บัญชี `virtual` เป็นข้อยกเว้นเพราะยอดคงเหลือของเงินทดรองจ่ายติดลบได้จากธุรกรรมภายหลัง
...
4. **ขอบเขตยอดติดลบ:**
   - เงินสด, บัญชีธนาคารออมทรัพย์ และบัญชีธนาคารกระแสรายวันที่ไม่มี OD ห้ามมียอดคงเหลือติดลบจากการโอนหรือจ่ายเงิน
   - บัญชีกระแสรายวันที่มี OD อาจมียอดคงเหลือจริงติดลบได้ แต่ต้องไม่เกินวงเงิน OD; ค่า `od_limit` เป็นวงเงินบวก ไม่ใช่ยอดติดลบ
   - บัญชี `virtual` ใช้เป็นยอดภาระเงินทดรองจ่าย เริ่มต้นที่ศูนย์หรือยอดไม่ติดลบ และอนุญาตให้ยอดคงเหลือติดลบเมื่อมี flow เงินทดรองจ่าย/เงินกู้รองรับ ไม่ใช่การกรอกยอดตั้งต้นติดลบ

5. **Concurrency Control:**
   - การตรวจยอดของ transfer, supplier payment และค่าใช้จ่ายแบบจ่ายทันทีทำภายใน transaction หลังล็อก advisory ระดับบัญชีแล้ว เพื่อให้คำขอพร้อมกันไม่สามารถใช้ยอดคงเหลือชุดเดียวกันซ้ำได้
