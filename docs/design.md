...
- **WYSIWYG (กติกากลาง บังคับทุกเอกสาร): Preview กับ Print ต้องเป็นเอกสารเดียวกันเป๊ะ — สิ่งที่เห็นใน popup preview คือสิ่งที่ออกจากเครื่องพิมพ์.** HTML ของทุก builder ห้ามมี `@media print` ที่บีบ layout ตอนพิมพ์ (ห้ามลด font-size, ห้ามลด padding เซลล์ตาราง/กล่อง/หัวเอกสาร, ห้ามลด margin ระยะระหว่างส่วน เช่น ช่องว่างก่อนลายเซ็น) — ถ้าเนื้อหาไม่พอต้องให้ตัวจัดหน้าทำงาน (ย้ายทั้งแถวไปหน้าถัดไป) ไม่ใช่บีบตอนพิมพ์. `@media print` ทำได้แค่: กล่องหน้า (A4 ลบ `@page` margin 8mm → 194×281mm, padding 0), ซ่อน toolbar preview, `print-color-adjust: exact`, และ rule เฉพาะการพิมพ์จริง (เช่น fixed print-footer ของ PO Sell). กลไกที่ทำให้ตรงกัน: popup จัดหน้า 1 ครั้งด้วย CSS ฐาน (screen) แล้ว print ใช้ DOM + CSS เดียวกัน → ตรงกันโดยโครงสร้าง. Regression guard: `expectPrintDoesNotShrink` ใน `document-print-contract.test.ts` + เช็คใน test ของ RV/PB/WTI; สคริปต์วัดจริง `reports/playwright/compare-sb-preview-print.mjs` เทียบตำแหน่ง/ขนาดพิกเซลทั้ง 9 เอกสาร (preview vs print media).
- ข้อยกเว้น WTI/WTO: ตารางในหน้าฟอร์มต่อเนื่องใช้เพดานเชิงตรรกะได้สูงสุด 20 รายการต่อหน้า แต่ความสูงจริงหลัง font/logo พร้อมเป็นตัวตัดสินเสมอ; แถวที่ทำให้ล้นต้องย้ายทั้งแถวไปหน้าถัดไป. หน้าสุดท้ายต้องสงวนยอดสรุปและลายเซ็น จึงอาจมีรายการน้อยกว่า 20 โดยตั้งใจ และหน้ารูปแนบไม่อยู่ใน capacity นี้. **ลำดับการเติม**: หน้า 1 และทุกหน้าฟอร์มต่อเนื่องต้องเต็ม 20 แถวก่อนเสมอ แล้วส่วนที่เหลือค่อยตกไปหน้าสุดท้าย (ห้าม pack แถวไปหน้าสุดท้ายก่อนแล้วเหลือหน้าแรกว่าง — ทั้งตัวจัดหน้า DOM `fillContinuationFirst` และ pure paginator ของ PDF/LINE ใช้กติกาเดียวกัน).
- WTI/WTO ใช้ฟอนต์ตารางขนาดเดียวทุกหน้า (10.5px base) — **ห้าม** มี variant ฟอนต์เล็กกว่า (เช่น dense 9px) บนหน้าที่มีแถวเยอะ เพราะทำให้เอกสารฉบับเดียวมีขนาดตัวอักษรไม่เท่ากัน; แถว heading ของกลุ่มสินค้า (`product-heading`) นับเป็น 1 ช่องของกริด 20 ช่องเท่านั้น (detail ที่ wrap อยู่ในเซลล์ ไม่ผลักแถวอื่นออกจากหน้า — browser fitter วัดความสูงจริงอีกครั้ง)
- มาตรฐานส่วนท้ายหน้าสุดท้ายของ WTI/WTO: `bottom-zone` ต้องยึดติดส่วนล่างของ A4 ด้วย `margin-top: auto` และเรียง `bottom-grid` (กรอบสรุป 3 ช่อง) อยู่เหนือตามด้วย `signatures` (4 ช่องลงนาม) เสมอ. ระยะ `bottom-grid` ถึง `signatures` คงที่ 28px ทั้ง Preview และ Print (WYSIWYG — ห้ามบีบตอนพิมพ์); เส้นเซ็นคงระยะภายในเดิม 16px. ห้ามเพิ่ม spacer, ยืดกรอบ, ทำให้ห่างหรือแคบกว่านี้, หรือย้ายสองส่วนนี้ไปตำแหน่งอื่นโดยไม่มีการอนุมัติจากผู้ใช้; เมื่อเนื้อหายาวให้ย้ายแถวตารางแทน.
- ตารางฟอร์ม WTI/WTO ใน Browser Preview/Print ต้องแบ่งความสูงของทุก row slot ตาม `page.capacity` เท่ากันเสมอ ทั้งแถวข้อมูลจริงและแถวว่าง; ห้ามบีบแถวว่างเพื่อให้แถวข้อมูลจริงเพียงแถวเดียวกินพื้นที่ที่เหลือ. ข้อความยาวขยายแถวได้ตามจำเป็น แล้วให้ตัวจัดหน้า DOM ย้ายทั้งแถวก่อนกระทบ `bottom-grid` หรือ `signatures`.
- ไฟล์ PDF (React-PDF) ของ WTI/WTO ต้องแสดงฟอร์ม 20 ช่องเหมือน Browser ทุกหน้า รวมหน้าสุดท้าย: ตารางใช้ `flexGrow: 1` เติมความสูง A4 ทั้งหมด แล้วแถวว่างแบ่งพื้นที่ที่เหลือเท่ากัน; ห้ามสงวนพื้นที่ bottom-zone ด้วย `marginBottom` บนตาราง (Yoga โต container ก่อนแล้วดัน bottom-zone ล้นหน้า) — การยืดต้องเกิดในแถว และ `bottom-zone` ต้องเป็น sibling ที่ยึดขอบล่างเสมอ.
- ข้อจำกัด react-pdf ที่ต้องรักษาไว้: unitless `lineHeight` บน `Page` ถูก resolve เป็น pt ตายตัว (12.4pt) แล้วสืบทอดไปทุก `Text` — แถวว่างต้องตั้ง `lineHeight` ของตัวเองให้เล็กจริงเสมอ ไม่งั้น 20 แถวว่างจะสูง 12.4pt/แถวจนล้น A4; `signatures` ใน PDF ต้องมี `marginTop: 18pt` (≈ 24px) เพื่อคงระยะจาก `bottom-grid` — pipeline PDF (React-PDF สำหรับ share) เป็นคนละตัวกับ browser print และคง contract เดิมของตัวเอง
...
- print preview ใช้พื้นหลังรอบกระดาษสี `#334155`; ตัวกระดาษเป็นสีขาว มีเงาและระยะห่างระหว่างหน้าในโหมด preview แต่ต้องตัดเงา/ระยะขอบ preview ออกเมื่อพิมพ์จริง
- ชื่อบริษัทตามกฎหมายรวม `(สำนักงานใหญ่)` ต้อง normalize newline และแสดงเป็นบรรทัดเดียวในหัวเอกสาร ไม่แยก suffix ไปอีกบรรทัด
...
- **Multi-Page Corporate Print Layout Standard (มาตรฐานการจัดหน้าเอกสารพิมพ์หลายหน้า):**
  - **เอกสารพิมพ์ทางการทั้งหมด:** ใช้ตัวจัดหน้ากลางที่วัดความสูงจริงของ DOM หลัง font และ logo พร้อม แล้วเรนเดอร์เป็นหน้า A4 แบบ `border-box`; 15 รายการเป็นจำนวนสูงสุดเชิงกายภาพต่อหน้า ไม่ใช่การรับประกันว่าต้องอยู่หน้าเดียว. แถวที่ทำให้กรอบล้นต้องย้ายทั้งแถวไปหน้าถัดไป และปุ่มพิมพ์ต้องรอจนทุกหน้าผ่าน overflow check.
  - **เอกสารหลายหน้า:**
    - **หน้า 1 ถึง หน้าก่อนสุดท้าย (Page 1..N-1):** แสดง Header (ระบุเลขหน้า เช่น `หน้า 1/2`) + ข้อมูลคู่ค้า/ผู้รับ/ผู้จ่ายเงิน + ตารางสินค้า + **คงพื้นที่ footer ตารางและกรอบ placeholder ตาม template โดยใส่หัวข้อที่กำหนดและค่า `-` แต่ห้ามแสดงยอดหรือหมายเหตุธุรกิจจริงระหว่างทาง** + วางข้อความ **`( มีต่อหน้า X / Continued on Page X ➔ )` ไว้ตรงกลางเด่นชัดแทนช่องเซ็นชื่อ** + หมายเหตุทางกฎหมาย
    - **หน้าสุดท้าย (Final Page):** แสดง Header (ระบุ `หน้า N/N`) + ข้อมูลคู่ค้า/ผู้รับ/ผู้จ่ายเงิน + ตารางสินค้าส่วนที่เหลือ (เติมแถวว่างตามพื้นที่จริง ไม่ฝืน 15 แถว) + **ตารางสรุปเงินสดสุทธิ (ลงตัวเลขจริงคำนวณสรุปและตัวอักษรบาทถ้วน)** + **ช่องลงลายมือชื่ออนุมัติเต็มรูปแบบ (`ผู้จ่ายเงิน` / `ผู้รับเงิน`)** + หมายเหตุทางกฎหมาย
    - **ความสมส่วนของโครงสร้าง:** ทุกหน้า (ทั้งหน้าแรก หน้ากลาง และหน้าสุดท้าย) ต้องมีสัดส่วน ความสูง และตำแหน่งกรอบตรงกัน 100% เพื่อความสวยงามในระดับ Corporate ERP Standard
  - **ข้อยกเว้นตามชนิดเอกสาร:** `PB` ใช้ตัวจัดหน้าและตัวแบ่ง REMARK แบบสองรอบของตัวเอง; `WTI/WTO` ใช้ capacity ของแบบฟอร์มและตัวจัดหน้า DOM เฉพาะหน้าฟอร์ม (หน้าอัลบั้มรูปไม่ถูกย้าย); รายงาน PMT/RCP ประจำวันยังคงงบหน้าแรก 8 แถวและไม่ใช้ตัวจัดหน้าเอกสารธุรกิจ; PMA ใช้ A4 landscape. เอกสารอื่นใช้ `prepareCorporatePrintLayout` กลางโดยไม่เปลี่ยน API/DB/Storage/Cache.
  - **หน้าอัลบั้มรูปของ WTI/WTO (หน้าอัลบั้มไม่ถูกตัวจัดหน้าย้าย):** หน้าอัลบั้มไม่มี `data-print-page` จึงไม่ถูก `normalizePrintPages` ปรับกรอบ แต่ต้องมีขนาดกระดาษเท่ากับหน้าฟอร์มที่ถูก normalize ทุกประการ: Preview ใช้กล่อง `210 × 297 มม.` `padding: 8mm` และ Print ใช้ `194 × 281 มม.` `padding: 0` (ตาม `@page` margin 8 มม.) — ห้ามให้หน้ารูปแคบ/เตี้ยกว่าหน้ารายการใน popup พิมพ์; regression guard ตรวจ CSS rule ของ `.attachment-page` ตรงกับค่า normalized.
  - **ความเร็ว popup พิมพ์ (ห้ามให้รูปถ่ายแนบเป็นตัวชะลอการจัดหน้า):** `prepareCorporatePrintLayout` ต้องรอเฉพาะ font + รูปที่ส่งผลต่อ layout (logo/รูปในหน้าฟอร์ม) ก่อนจัดหน้า — รูปในหน้าอัลบั้ม (`.attachment-page`) ไม่ถูกวัดโดยตัวจัดหน้า จึงให้โหลดแบบขนานและรอแค่ก่อนเปลี่ยนสถานะเป็น "พร้อมพิมพ์" เท่านั้น (ปุ่มพิมพ์ยังต้องรอรูปครบทุกใบ เพื่อไม่ให้พิมพ์ได้รูปครึ่งใบ). เวลาโฮเวอร์ปุ่มพิมพ์ให้ warm font/โปรไฟล์บริษัท/logo และดึงรายละเอียดเอกสารพร้อม preload รูปล่วงหน้า; timeout ใช้ deadline ร่วมกันเดียวตลอดรอบจัดหน้า ห้ามขยายเวลารอรวม. Regression guard: test "paginates form rows without waiting for album photo images" ใน `corporate-print-layout.test.ts`.
  - ตาราง PB ต้องแยกค่าตัวเลขออกจากคอลัมน์ `หน่วย` อย่างชัดเจน: `นน.ก่อนหัก`, `นน.หัก`, `นน.สุทธิ` เป็นตัวเลขล้วน และ `หน่วย` แสดง `กก.`/หน่วยจริงของบรรทัดรวมถึงยอดท้ายตาราง; ห้ามต่อหน่วยเข้ากับตัวเลขจนชนคอลัมน์ข้างเคียง.
  - REMARK ของ PB ที่เป็นรายการหลายข้อต้องแสดง `1.`, `2.`, `3.` ต่อเนื่องคนละบรรทัดด้วย hanging indent และแสดงครบทุกตัวอักษร. ถ้ารายการเดียวสูงเกินหน้า ให้แบ่งได้เฉพาะระหว่างข้อ; segment หน้าต่อใช้เลขรายการเดิมและชื่อสินค้า `(ต่อ)` โดยไม่ทำค่าน้ำหนัก ราคา หรือยอดเงินซ้ำ. หมายเหตุธรรมดาหนึ่งข้อความไม่เติมเลขโดยไม่จำเป็น.
  - Preview PB ใช้ A4 `210 × 297 มม.` แบบ `border-box` และ Print ใช้ content box `194 × 281 มม.` ตาม `@page` margin 8 มม. ก่อนวัดต้องบังคับโหลดและยืนยัน Noto Sans Thai ทั้ง Regular/Bold รวมถึง logo; Browser ที่ตรวจฟอนต์ไม่ได้, โหลดไม่สำเร็จ, timeout หรือวัดไม่ได้ต้องแสดงข้อผิดพลาดและห้ามเปิด Print. ปุ่มพิมพ์ต้อง disabled จนจัดหน้าและตรวจ `scrollHeight`/`scrollWidth` ทุกหน้าแล้วว่าไม่ล้น.
