...
- ข้อยกเว้น WTI/WTO: ตารางในหน้าฟอร์มต่อเนื่องใช้เพดานเชิงตรรกะได้สูงสุด 20 รายการต่อหน้า แต่ความสูงจริงหลัง font/logo พร้อมเป็นตัวตัดสินเสมอ; แถวที่ทำให้ล้นต้องย้ายทั้งแถวไปหน้าถัดไป. หน้าสุดท้ายต้องสงวนยอดสรุปและลายเซ็น จึงอาจมีรายการน้อยกว่า 20 โดยตั้งใจ และหน้ารูปแนบไม่อยู่ใน capacity นี้.
- มาตรฐานส่วนท้ายหน้าสุดท้ายของ WTI/WTO: `bottom-zone` ต้องยึดติดส่วนล่างของ A4 ด้วย `margin-top: auto` และเรียง `bottom-grid` (กรอบสรุป 3 ช่อง) อยู่เหนือตามด้วย `signatures` (4 ช่องลงนาม) เสมอ. ระยะ `bottom-grid` ถึง `signatures` คงที่ที่ 28px ใน Preview และ 24px ตอน Print; เส้นเซ็นคงระยะภายในเดิม 16px/12px ตามลำดับ. ห้ามเพิ่ม spacer, ยืดกรอบ, ทำให้ห่างหรือแคบกว่านี้, หรือย้ายสองส่วนนี้ไปตำแหน่งอื่นโดยไม่มีการอนุมัติจากผู้ใช้; เมื่อเนื้อหายาวให้ย้ายแถวตารางแทน.
...
- ข้อจำกัด react-pdf ที่ต้องรักษาไว้: unitless `lineHeight` บน `Page` ถูก resolve เป็น pt ตายตัว (12.4pt) แล้วสืบทอดไปทุก `Text` — แถวว่างต้องตั้ง `lineHeight` ของตัวเองให้เล็กจริงเสมอ ไม่งั้น 20 แถวว่างจะสูง 12.4pt/แถวจนล้น A4; `signatures` ใน PDF ต้องมี `marginTop: 18pt` (≈ 24px print) เพื่อคงระยะจาก `bottom-grid`
