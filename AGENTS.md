...
- Before starting any task, discuss and plan with the user first to confirm mutual understanding (scope, approach, target files, and how to validate).
- Do NOT begin implementation until the user explicitly says to start (e.g. "เริ่ม", "start", "ทำเลย"). Present the plan and wait for approval.
- If the user changes direction mid-task, stop and re-confirm the plan before continuing.
...
- **Mandatory Code Review:** After completing implementation and verification for every task/issue, you MUST perform a rigorous Code Review (checking type signatures, null safety, contract compliance, UI alignment, and edge cases) and present a Code Review Report before claiming completion.
