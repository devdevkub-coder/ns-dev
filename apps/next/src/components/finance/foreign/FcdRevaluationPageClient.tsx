...
export function FcdRevaluationPageClient() {
  ...
  return <section className="space-y-4" data-ns-field-scope="entry">
    ...
    <div className="grid grid-cols-2 gap-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-3">
      ...
      <div className="col-span-2 md:col-span-1"><Field label="บัญชี FCD และสกุลเงิน"><Select value={form.account} onChange={(event) => setForm({ ...form, account: event.target.value })}><option value="">เลือกบัญชี FCD</option>{(data?.filters.accountCurrencies ?? []).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</Select></Field></div>
      ...
      <div className="col-span-2 md:col-span-1"><Field label="เหตุผลเมื่อกรอก/แก้ rate เอง"><input className="h-10 w-full rounded-md border border-slate-300 px-2 text-sm" value={form.rateOverrideReason} onChange={(event) => setForm({ ...form, rateOverrideReason: event.target.value })} /></Field></div>
      <div className="col-span-2 flex items-end md:col-span-1"><button className="h-10 w-full rounded-md bg-emerald-600 px-4 text-sm font-semibold text-white disabled:opacity-50" disabled={saving} type="button" onClick={() => void submit()}>Post การตีมูลค่า</button></div>
    </div>
    ...
    {columnResize.hasCustomWidths ? <div className="flex justify-end"><button className="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700 hover:bg-slate-50 hidden lg:inline-flex" type="button" onClick={columnResize.resetColumnWidths}>คืนค่าเดิมตาราง</button></div> : null}
    <div className="hidden overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm lg:block">
      <div className="overflow-x-auto">
        <table className="ns-table min-w-full text-sm" style={{ minWidth: columnResize.tableMinWidth, maxWidth: columnResize.tableMaxWidth, tableLayout: 'fixed', width: '100%' }}>
          ...
          <tbody>
            {isLoading ? <tr><td className="p-8 text-center text-slate-500" colSpan={revaluationColumns.length}><div className="flex items-center justify-center gap-3 py-1"><Skeleton className="h-4 w-44" /><Skeleton className="h-4 w-28" /><span className="sr-only">กำลังโหลดข้อมูล</span></div></td></tr> : null}
