...
export function ResizableTableHead<TSortKey extends string>({
  ...
  return (
    ...
    >
      {sortKey && onSort ? (
        <button className={`flex w-full min-w-0 items-center ${alignment.justify} gap-3 ${contentPadding} ${alignment.text} hover:bg-slate-200 focus-visible:relative focus-visible:z-10 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-inset focus-visible:ring-blue-500/40`} type="button" onClick={() => onSort(sortKey)}>
          {content}
        </button>
      ) : (
        <div className={`flex min-w-0 items-center ${alignment.justify} gap-3 ${contentPadding} ${alignment.text}`}>
          {content}
        </div>
      )}
      {resizeProps ? (
