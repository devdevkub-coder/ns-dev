...
describe('Allocation Ledger table density', () => {
  ...
  it('opens matched-cost details from the allocated quantity while keeping the group dropdown available', () => {
    ...
    expect(view).toContain('setSelectedDetailKey(row.matchId)')
    expect(view).toContain('onClick={() => setSelectedDetailKey(row.matchId)}')
  ...
  it('groups the main ledger by match id and exposes a dropdown for each source row', () => {
    ...
    expect(view).toContain('rowsByMatch.get(selectedDetailKey)')
