...
export function WeightTicketStockReturnDialog({
  ...
  const [noteByPendingOut, setNoteByPendingOut] = useState<Record<string, string>>({})
  ...
  async function submitReturn(option: StockReturnOption) {
    ...
    try {
      const response = await fetch(`/api/sales/bills/${encodeURIComponent(salesBillDocNo)}/stock-return`, {
        body: JSON.stringify({
          note: noteByPendingOut[option.pendingOutKey]?.trim() || null,
      ...
      <DialogContent aria-labelledby="wto-stock-return-title" className="max-h-[90vh] max-w-6xl overflow-hidden rounded-md p-0 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50 p-4">
          <div className="mb-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
            กรอกน้ำหนักที่ชั่งคืนจริง ถ้าน้อยกว่า pending_out ต้องระบุเหตุผล ระบบจะบันทึกส่วนต่างเป็นของขาดและลง Stock Ledger
          </div>
                  ...
                  {options.map((option) => {
                    ...
                    return (
                      <tr key={option.pendingOutKey} className="border-t border-slate-100 align-top">
                        ...
                        <td className="px-3 py-2">
                          <input
                            className={`w-full rounded-md border px-2 py-2 ${requiresReason && !reasonByPendingOut[option.pendingOutKey]?.trim() ? 'border-red-300 bg-red-50' : 'border-slate-300 bg-white'}`}
                        ...
                        <td className="px-3 py-2">
                          <input
                            ...
                            value={noteByPendingOut[option.pendingOutKey] ?? ''}
                            onChange={(event) => setNoteByPendingOut((current) => ({ ...current, [option.pendingOutKey]: event.target.value }))}
                          />
                        </td>
