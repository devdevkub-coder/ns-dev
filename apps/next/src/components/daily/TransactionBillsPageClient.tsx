...
import { invalidatePrintDocumentDetail, peekPrintDocumentDetail, prefetchPrintAssets, prefetchPrintDocumentDetail } from '@/lib/print-asset-prefetch'
...
import {
  ...
  type SortDirection,
  type SortKey,
} from '@/components/daily/TransactionBillsDetailModals'
...
export function TransactionBillsPageClient({ mode }: TransactionBillsPageClientProps) {
    ...
    try {
      if (mode === 'purchase') {
        ...
        const [payload, scopedReferences, globalReferences] = await Promise.all([
          ...
          cachedSalesBillReferences<SalesScopedReferencePayload>('/api/sales/bills/options?scope=reference'),
  ...
  async function correctTradingAllocations(docNo: string, allocations: Array<{ salesLineNo: number; tradingCostSourceId: string }>, note: string) {
    ...
    invalidatePrintDocumentDetail(docNo)
    await Promise.all([loadData(), reloadSalesDetail(docNo)])
  }

  /**
   * Warm everything the print popup needs while the user hovers over the
   * print action: the company profile/fonts and (for bills) the full detail
   * payload, so a click renders the popup without waiting on a ~1s API call.
   */
  function warmBillPrintDetail(row: BillRow, kind: 'sales' | 'purchase') {
    if (row.branchId) void prefetchPrintAssets(row.branchId)
    void prefetchPrintDocumentDetail<SalesBillDetail | PurchaseBillDetail>(row.docNo, () => (
      kind === 'sales'
        ? dailyFetchJson<SalesBillDetail>(`/api/sales/bills/${encodeURIComponent(row.docNo)}`)
        : dailyFetchJson<PurchaseBillDetail>(`/api/purchase/bills/${encodeURIComponent(row.docNo)}`)
    ))
  }
  ...
  async function printPurchaseBill(rowOrDetail: BillRow | PurchaseBillDetail) {
    ...
    try {
      ...
      // Start the company profile fetch now (parallel with the detail fetch)
      // so the popup does not wait for a second ~1s API round trip inside
      // `openPurchaseBillPrint`. The detail itself may already be in the
      // hover prefetch cache.
      const profileWarm = rowOrDetail.branchId ? prefetchPrintAssets(rowOrDetail.branchId) : null
      const detail = isPurchaseBillDetail(rowOrDetail)
        ? rowOrDetail
        : (peekPrintDocumentDetail<PurchaseBillDetail>(docNo)
  ...
  async function printSalesBill(rowOrDetail: BillRow | SalesBillDetail) {
    ...
    try {
      ...
      // Start the company profile fetch now (parallel with the detail fetch)
      // so the popup does not wait for a second ~1s API round trip inside
      // `openSalesBillPrint`. The detail itself may already be in the hover
      // prefetch cache.
      const profileWarm = rowOrDetail.branchId ? prefetchPrintAssets(rowOrDetail.branchId) : null
      const detail = isSalesBillDetail(rowOrDetail)
        ? rowOrDetail
        : (peekPrintDocumentDetail<SalesBillDetail>(docNo)
              ...
              {isCancelledBillRow(row) ? null : (
                <div className="flex justify-end mt-3 pt-2 border-t border-slate-100/60" onClick={(e) => e.stopPropagation()}>
                  <TableActionButton mobileLabel menu={(
                    <>
                      {mode === 'purchase' ? <TableActionMenuItem disabled={printingBillDocNo === row.docNo} onMouseEnter={() => warmBillPrintDetail(row, 'purchase')} onFocus={() => warmBillPrintDetail(row, 'purchase')} onSelect={() => void printPurchaseBill(row)}>พิมพ์</TableActionMenuItem> : null}
                      ...
                      {mode === 'sales' ? <TableActionMenuItem disabled={printingBillDocNo === row.docNo} onMouseEnter={() => warmBillPrintDetail(row, 'sales')} onFocus={() => warmBillPrintDetail(row, 'sales')} onSelect={() => void printSalesBill(row)}>พิมพ์</TableActionMenuItem> : null}
              ...
              <TableRow key={row.id} className={`${row.status === 'cancelled' ? 'bg-red-100/60 hover:bg-red-200/60 text-slate-400' : 'hover:bg-slate-50'} cursor-pointer`} onClick={() => openListRow(row)}>
                ...
                {mode === 'purchase' ? (
                  <td className="whitespace-nowrap p-2 text-center">
                    {isCancelledBillRow(row) ? null : <TableActionButton menu={(
                      <>
                        <TableActionMenuItem disabled={printingBillDocNo === row.docNo} onMouseEnter={() => warmBillPrintDetail(row, 'purchase')} onFocus={() => warmBillPrintDetail(row, 'purchase')} onSelect={() => void printPurchaseBill(row)}>พิมพ์</TableActionMenuItem>
                ...
                {mode === 'sales' ? (
                  <td className="whitespace-nowrap p-2 text-center">
                    {isCancelledBillRow(row) ? null : <TableActionButton menu={(
                      <>
                        <TableActionMenuItem disabled={printingBillDocNo === row.docNo} onMouseEnter={() => warmBillPrintDetail(row, 'sales')} onFocus={() => warmBillPrintDetail(row, 'sales')} onSelect={() => void printSalesBill(row)}>พิมพ์</TableActionMenuItem>
