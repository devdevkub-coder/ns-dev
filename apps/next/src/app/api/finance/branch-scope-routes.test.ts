...
describe.each(routes)('$name finance destination branch scope', ({ agingBill, billFindMany, get, partyFindMany, partyRelation, url }) => {
  ...
  it('fail-closes an all-branch request to the effective allowed codes', async () => {
    ...
    expect(partyFindMany).toHaveBeenCalledWith(expect.objectContaining({
      select: expect.objectContaining({
        [partyRelation]: expect.objectContaining({
          where: { active: true, branches: { is: { code: { in: ['B01'] } } } },
        }),
      }),
      where: {
        active: true,
        [partyRelation]: { some: { active: true, branches: { is: { code: { in: ['B01'] } } } } },
      },
    }))
  })
  ...
  it('keeps a branchScope=all role unrestricted', async () => {
    ...
    expect(partyFindMany).toHaveBeenCalledWith(expect.objectContaining({
      select: expect.objectContaining({ [partyRelation]: expect.objectContaining({ where: { active: true } }) }),
      where: { active: true },
    }))
  })
