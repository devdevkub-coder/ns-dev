...
export async function buildDailyReportDashboard(filter: MainDashboardFilter): Promise<DailyReportPayload> {
  ...
  for (const product of products) {
    productByKey.set(String(product.id), product)
    productByKey.set(product.code, product)
  }
  ...
  const salesAmount = activeSales.reduce((sum, row) => sum + salesBillRevenueAmount(row), 0)
  ...
  const grossProfit = activeSales.reduce((sum, row) => sum + salesBillGrossProfitAmount(row), 0)
  ...
  for (const bill of activeSales) for (const item of salesLineRows(linesByBill.get(bill.id))) { const product = productByKey.get(item.productId); const current = productOut.get(item.productId) ?? { amount: 0, code: product?.code ?? '', group: product?.metal_group ?? 'อื่นๆ', id: product?.code ?? '', name: product?.name ?? '-', qty: 0 }; current.amount += item.amount; current.qty += item.qty; productOut.set(item.productId, current) }
  ...
  for (const bill of activeSales) { const key = bill.customers?.code ?? bill.customers?.name ?? ''; const current = topCustomers.get(key) ?? { amount: 0, bills: 0, gp: 0, id: bill.customers?.code ?? '', name: bill.customers?.name ?? '-', qty: 0 }; current.amount += salesBillRevenueAmount(bill); current.gp += salesBillGrossProfitAmount(bill); current.qty += salesBillAnalyticsLineTotals(linesByBill.get(bill.id)).qty; current.bills += 1; topCustomers.set(key, current) }
  ...
  for (const bill of todayPurchases) for (const item of itemRows(purchaseBillItemRows(bill))) { const product = productByKey.get(item.productId); const group = getGroup(product?.metal_group ?? 'อื่นๆ'); const row = group.products.get(item.productId) ?? { buyAmt: 0, buyQty: 0, productCode: product?.code ?? '', productId: product?.code ?? '', productName: product?.name ?? '-', sellAmt: 0, sellQty: 0 }; group.products.set(item.productId, row); group.buyAmt += item.amount; group.buyQty += item.qty; row.buyAmt += item.amount; row.buyQty += item.qty }
  for (const bill of todaySales) for (const item of salesLineRows(linesByBill.get(bill.id))) { const product = productByKey.get(item.productId); const group = getGroup(product?.metal_group ?? 'อื่นๆ'); const row = group.products.get(item.productId) ?? { buyAmt: 0, buyQty: 0, productCode: product?.code ?? '', productId: product?.code ?? '', productName: product?.name ?? '-', sellAmt: 0, sellQty: 0 }; group.products.set(item.productId, row); group.sellAmt += item.amount; group.sellQty += item.qty; row.sellAmt += item.amount; row.sellQty += item.qty }
  ...
  for (const bill of activeSales) { const label = toDateOnly(bill.date); const row = dailyTrend.get(label) ?? { label, purchase: 0, sales: 0 }; row.sales += salesBillRevenueAmount(bill); dailyTrend.set(label, row) }
  ...
  return {
    ...
    dailyReport: {
      ...
      expenseByCategory: Array.from(expenseByCategory.values()).sort((a, b) => b.amount - a.amount), expenseRows: todayExpenses.slice(0, 12).map((row) => ({ amount: toNumber(row.amount), category: row.expense_categories?.name ?? '-', docNo: row.doc_no, payee: row.payee ?? '-' })), groupBreakdown, purchaseBills: todayPurchases.slice(0, 12).map((row) => ({ amount: toNumber(row.total_amount), docNo: row.doc_no, name: row.suppliers?.name ?? '-', qty: purchaseBillItemQty(row) })), salesBills: todaySales.slice(0, 12).map((row) => ({ amount: salesBillRevenueAmount(row), docNo: row.doc_no, name: row.customers?.name ?? '-', qty: salesBillAnalyticsLineTotals(linesByBill.get(row.id)).qty })), summary: { expenseAmount: todayExpenses.reduce((sum, row) => sum + toNumber(row.amount), 0), purchaseAmount: todayPurchases.reduce((sum, row) => sum + toNumber(row.total_amount), 0), purchaseCount: todayPurchases.length, purchaseQty: todayPurchases.reduce((sum, row) => sum + purchaseBillItemQty(row), 0), salesAmount: todaySales.reduce((sum, row) => sum + salesBillRevenueAmount(row), 0), salesCount: todaySales.length, salesQty: todaySales.reduce((sum, row) => sum + salesBillAnalyticsLineTotals(linesByBill.get(row.id)).qty, 0) },
    },
  }
}
