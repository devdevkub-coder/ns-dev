...
function getTradingLabel(sourceLabel: string | null | undefined) {
  if (!sourceLabel) return ''
  return sourceLabel.split('/').map((part) => part.trim()).find((part) => (
    part.startsWith('Trading PB') || part.startsWith('Cost Source') || part.startsWith('รอ Trading allocation')
  )) ?? ''
}

function itemRows(items: SalesBillDetail['items'], startIndex: number) {
...
function emptyRows(count: number) {
  return Array.from({ length: Math.max(0, count) }, (_, index) => (
    `<tr class="empty" data-row-slot="empty-${index + 1}"><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td></tr>`
  )).join('')
}
...
export function buildSalesBillPrintHtml(bill: SalesBillDetail, profile: CompanyProfilePrintValues) {
  ...
  const unitTotals = totalsByUnit(bill)
  const qtySummaryText = unitTotals.map((item) => `${money(item.qty)} ${item.unit}`).join(' / ') || '-'
  const amountSummaryText = unitTotals.map((item) => `${money(item.amount)} บาท`).join(' / ') || '-'
  const lineDiscountTotal = bill.items.reduce((sum, item) => sum + (item.discount ?? 0), 0)
  const pages = paginateStandardPrintItems(bill.items)
        ...
        <section class="section-grid">
          ...
          <div class="panel">
            ...
            <div class="panel-body two-col">
              ...
              <div style="grid-column:1 / -1"><div class="field-label">ใบส่งของ WTO</div><div class="field-value">${escapeHtml(bill.deliveryDocNos.join(', ') || '-')}</div></div>
            </div>
          </div>
        </section>
        ...
        <table class="items">
          <thead>
            <tr>
              ...
              <th style="width:78mm">สินค้า${page.pageNo > 1 ? ' (ต่อ)' : ''}</th>
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${escapeHtml(title)} ${escapeHtml(bill.docNo)}</title>
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .doc-title { font-size: 22px; font-weight: 900; color: #0f766e; }
      .page-label { margin-top: 4px; color: #0f766e; font-weight: 700; }
      .doc-status { display: inline-flex; margin-top: 5px; border-radius: 999px; background: #f1f5f9; color: #334155; padding: 3px 9px; font-weight: 700; }
      ...
      .field-value { font-weight: 700; color: #0f172a; margin-top: 1px; overflow-wrap: anywhere; }
