...
const salesLineItemBaseSchema = z.object({
...
export const salesLineItemSchema = salesLineItemBaseSchema.extend({
  price: positiveNumber('ราคาขาย/หน่วย'),
  qty: positiveNumber('จำนวน'),
})

export const salesBillEditLineItemSchema = salesLineItemBaseSchema.extend({
  price: money('ราคาขาย/หน่วย'),
  qty: money('จำนวน'),
}).refine((value) => value.qty <= 0.0001 || value.price > 0, {
  message: 'ราคาขาย/หน่วยต้องมากกว่า 0 เมื่อมีจำนวนขาย',
  path: ['price'],
})

function buildSalesBillFormSchema(itemSchema: typeof salesLineItemSchema | typeof salesBillEditLineItemSchema) {
  return z.object({
    ...
    items: z.array(itemSchema).min(1, 'เพิ่มรายการสินค้าอย่างน้อย 1 รายการ').max(50, 'รายการสินค้ามากเกินไป'),
...
export const salesBillFormSchema = buildSalesBillFormSchema(salesLineItemSchema)
export const salesBillEditFormSchema = buildSalesBillFormSchema(salesBillEditLineItemSchema)
...
export type SalesBillEditFormValues = z.infer<typeof salesBillEditFormSchema>
